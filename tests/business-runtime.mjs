...
try{
  await page.goto(baseUrl,{waitUntil:'commit',timeout:15000});await page.waitForTimeout(2000);
  const contract=await page.evaluate(()=>({keys:Object.keys(window.__KFE_RUNTIME||{}),models:Object.keys(window.KFE_VIEW_MODELS||{}),actions:Object.keys(window.__KFE_RUNTIME?.actions||{}),vue:!!window.KFE_VUE_RUNTIME,initialTab:document.querySelector('.tab-panel.active')?.id||null,bootError:window.KFE_BOOT_ERROR||null}));
  if(!contract.keys.length)fail(`Runtime contract was not published. Diagnostics=${JSON.stringify(contract)} Errors=${errors.join(' | ')}`);
  for(const name of ['workViewModel','fuelViewModel','expensesViewModel','revenueViewModel'])if(!contract.keys.includes(name))fail(`Runtime contract missing ${name}`);
  for(const name of ['work','fuel','expenses','revenue','maintenance','loan','renewals'])if(!contract.models.includes(name))fail(`Missing screen view-model: ${name}`);
  ...
  await page.evaluate(()=>window.__KFE_RUNTIME__.actions.endWork({endOdo:10050,now:1700003600000}));await page.waitForTimeout(100);
  const afterEnd=await page.evaluate(async()=>({state:await window.KFE_REPOSITORY.load(),work:window.__KFE_RUNTIME__.workViewModel,dashboard:window.__KFE_RUNTIME__.dashboardViewModel,domKm:document.querySelector('#today-km')?.textContent?.trim(),dashKm:document.querySelector('#dash-km')?.textContent?.trim()}));
  if(afterEnd.state?.work?.endOdo!==10050||afterEnd.state?.work?.status!=='Closed')fail(`Repository-backed end failed: ${JSON.stringify(afterEnd.state)}`);if(afterEnd.work?.km!==50)fail(`Work view-model mileage failed: ${JSON.stringify(afterEnd.work)}`);if(afterEnd.dashboard?.work?.km!==50)fail(`Dashboard did not consume aggregated work view-model: ${JSON.stringify(afterEnd.dashboard)}`);if(afterEnd.domKm!=='50')fail(`Existing Work UI did not consume work view-model after mutation: ${afterEnd.domKm}`);if(afterEnd.dashKm!=='50 km')fail(`Existing Dashboard UI did not consume dashboard view-model: ${afterEnd.dashKm}`);
