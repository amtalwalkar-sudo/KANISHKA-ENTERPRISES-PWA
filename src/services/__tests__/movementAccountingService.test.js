...
describe("MovementAccountingService", () => {
  ...
  it("upgrades only manually reconciled Uber Business KM to authoritative", async () => {
    ...
    const trip1 = result.segments.find((segment) => segment.tripId === "trip-1"); const trip2 = result.segments.find((segment) => segment.tripId === "trip-2")
  ...
  it("flags an over-estimated GPS/manual movement total instead of fabricating a negative remainder", async () => {
    ...
    expect(result.reconciliationStatus).toBe("OVER_ESTIMATE"); expect(result.unclassifiedKm).toBe(0); expect(result.reconciliationDifferenceKm).toBeLessThan(0)
  })
  it("handles a missing trip endpoint without fabricating distance", async () => {
    ...
    expect(missing.segments[1].distanceKm).toBeNull(); expect(missing.deadMilesKm).toBeGreaterThanOrEqual(0)
  })
  it("removes stationary GPS jitter before movement accounting", () => {
    const trace = filterGpsTrace([{ latitude: 19, longitude: 72, accuracy: 5, speed: 0, capturedAt: "2026-09-12T08:00:00Z" }, { latitude: 19.00005, longitude: 72.00002, accuracy: 5, speed: 0, capturedAt: "2026-09-12T08:01:00Z" }, { latitude: 19.001, longitude: 72, accuracy: 5, speed: 10, capturedAt: "2026-09-12T08:02:00Z" }])
    expect(trace).toHaveLength(2); expect(trace[1].speed).toBe(10)
  })
  ...
  it("rejects a backwards odometer and handles a zero-distance shift without negative remainder", async () => {
    ...
    expect(result.totalShiftVehicleKm).toBe(0); expect(result.unclassifiedKm).toBe(0); expect(result.reconciliationStatus).toBe("RECONCILED")
  })
})
