...
const readPosition = (options = {}) => new Promise((resolve) => {
  ...
  navigator.geolocation.getCurrentPosition((position) => resolve({ latitude: Number(position.coords.latitude), longitude: Number(position.coords.longitude), accuracy: Number(position.coords.accuracy), speed: Number.isFinite(position.coords.speed) ? Number(position.coords.speed) : null, bearing: Number.isFinite(position.coords.heading) ? Number(position.coords.heading) : null, timestamp: new Date(position.timestamp || Date.now()).toISOString() }), () => resolve(null), { enableHighAccuracy: true, timeout: 8000, maximumAge: 60000, ...options })
})
...
const capture = async (handler, force = false) => { const now = Date.now(); if (!force && now - lastSnapshotAt < GPS_INTERVALS_MS[currentState]) return null; const location = await readPosition(); if (location) { lastSnapshotAt = now; await handler(location) } return location }
const schedule = () => { if (timer !== null) window.clearTimeout(timer); timer = window.setTimeout(async () => { if (activeHandler) await capture(activeHandler); if (activeHandler) schedule() }, GPS_INTERVALS_MS[currentState]) }
...
export const LocationService = {
  ...
  stopActiveShiftSnapshots() { if (timer !== null) window.clearTimeout(timer); timer = null; activeHandler = null; ActivityDetectionService.stop(); void stopAndroidForegroundService() },
