...
const readPosition = (options = {}) => new Promise((resolve) => {
  ...
  }), () => resolve(null), { enableHighAccuracy: true, timeout: 8000, maximumAge: 60000, ...options })
})
...
const stopAndroidForegroundService = async () => {
  try { if (foregroundService) await foregroundService.stopForegroundService() } catch (error) { console.warn('Unable to stop Android foreground location service.', error) } finally { foregroundService = null }
}

const applyMovementState = (location) => {
  if (!location || currentState === 'WAITING' || currentState === 'TRIP_ACTIVE') return
  const speed = Number(location.speed)
  const next = Number.isFinite(speed) && speed > MOVEMENT_SPEED_KMH ? 'BETWEEN_TRIPS_MOVING' : 'BETWEEN_TRIPS_STATIONARY'
...
const schedule = () => {
  ...
  if (!activeHandler || typeof window === 'undefined') return
...
export const LocationService = {
  ...
  startActiveShiftSnapshots(handler, state = 'WAITING') {
    ...
    void ActivityDetectionService.start(() => {
      if (currentState === 'BETWEEN_TRIPS_STATIONARY') this.setState('BETWEEN_TRIPS_MOVING')
    })
