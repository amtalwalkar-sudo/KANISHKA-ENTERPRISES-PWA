...
const normalizeLocation = (location) => {
  ...
  return {
    ...
    speed: finite(location.speed) ? Number(location.speed) : null,
    bearing: finite(location.bearing) ? Number(location.bearing) : null,
    capturedAt: location.capturedAt || location.timestamp || null
  }
}
...
const orderedTrace = (snapshots = []) => [...snapshots]
  .filter(Boolean)
...
export const filterGpsTrace = (points = []) => {
  ...
  for (const point of orderedTrace(points)) {
    ...
    const displacement = haversineDistanceKm(previous, point) || 0
    const speed = Number(point.speed)
    const stationary = Number.isFinite(speed) ? speed <= STATIONARY_SPEED_KMH : false
    if (stationary && displacement <= JITTER_DISTANCE_KM) continue
    filtered.push(point)
  }
  return filtered
}
...
const waitingAnchorForTrace = (points = []) => {
  const trace = filterGpsTrace(points)
  if (!trace.length) return null
  let anchor = trace[0]
  for (let i = 1; i < trace.length; i += 1) {
    const point = trace[i]
    const speed = Number(point.speed)
    if (Number.isFinite(speed) && speed > STATIONARY_SPEED_KMH) break
    anchor = point
  }
  return anchor
}
...
const sum = (segments, classification) => segments
  .filter(s => s.classification === classification && finite(s.distanceKm))
  .reduce((t, s) => t + Number(s.distanceKm), 0)
...
const pointsBetween = (snapshots, startAt, endAt, from, to) => {
  ...
  for (const snapshot of orderedTrace(snapshots)) {
    const t = new Date(snapshot.capturedAt || 0).getTime()
    if ((!startAt || t >= new Date(startAt).getTime()) && (!endAt || t <= new Date(endAt).getTime())) points.push(snapshot)
  }
  if (b) points.push({ ...b, capturedAt: endAt || new Date().toISOString() })
  return filterGpsTrace(points)
}

const routeTrace = async (trace, router) => {
  const points = filterGpsTrace(trace)
  if (points.length < 2) return { distanceKm: null, method: 'UNAVAILABLE', confidence: 'UNAVAILABLE', geometry: [], provenance: null, points }
  try {
    const routed = typeof router?.routeTrace === 'function'
      ? await router.routeTrace(points)
      : typeof router === 'function' ? await router(points[0], points[points.length - 1], points) : null
    if (routed && finite(routed.distanceKm) && Number(routed.distanceKm) >= 0) {
      return {
        distanceKm: Number(routed.distanceKm),
        method: routed.method || 'ROUTED_TRACE',
        confidence: routed.confidence || 'ESTIMATED_ROAD_TRACE',
        geometry: Array.isArray(routed.geometry) ? routed.geometry : [],
        provenance: routed.provenance || { provider: routed.provider || 'UNKNOWN', method: routed.method || 'ROUTED_TRACE' },
        points
      }
    }
  } catch (error) {
    console.warn('Routing engine unavailable; falling back to filtered GPS trace.', error)
  }
  let distanceKm = 0
  for (let i = 1; i < points.length; i += 1) distanceKm += haversineDistanceKm(points[i - 1], points[i]) || 0
  return { distanceKm, method: 'FILTERED_GPS_TRACE', confidence: 'ESTIMATED_TRACE', geometry: points.map(p => [p.longitude, p.latitude]), provenance: { provider: 'KFE_TRACE_FALLBACK', method: 'FILTERED_GPS_TRACE' }, points }
}

const makeSegment = async ({ from, to, classification, label, router, tripId = null, trace = [] }) => {
  const filteredTrace = filterGpsTrace(trace)
  const routeResult = await routeTrace(filteredTrace, router)
  const fallbackFrom = normalizeLocation(from); const fallbackTo = normalizeLocation(to)
  let result = routeResult
  if (result.distanceKm === null && fallbackFrom && fallbackTo) {
    result = { distanceKm: haversineDistanceKm(fallbackFrom, fallbackTo), method: 'HAVERSINE_ENDPOINT_FALLBACK', confidence: 'ESTIMATED_STRAIGHT_LINE', geometry: [fallbackFrom, fallbackTo], provenance: { provider: 'KFE_ENDPOINT_FALLBACK', method: 'HAVERSINE_ENDPOINT_FALLBACK' }, points: filteredTrace }
  }
  return {
    ...
    gpsTracePoints: result.points.length,
    filteredGpsTrace: result.points,
    roadMatchedGeometry: result.geometry,
    routingProvenance: result.provenance,
    waitingAnchor: waitingAnchorForTrace(filteredTrace)
  }
}
...
export const MovementAccountingService = {
  ...
  async calculateSegments({ garageLocation, trips = [], router, gpsSnapshots = [] } = {}) {
    ...
    const snapshots = orderedTrace(gpsSnapshots)
    const segments = []
    ...
    return {
      ...
      routingProvenance: segments.map(s => ({ segmentId: s.id, ...s.routingProvenance })),
      roadMatchedGeometry: segments.map(s => ({ segmentId: s.id, geometry: s.roadMatchedGeometry }))
    }
  },
  ...
  async reconcileShiftMovement({ garageLocation, trips = [], startOdometer, endOdometer, router, manualBusinessKmByTripId = {}, gpsSnapshots = [] } = {}) {
    ...
    const base = await this.calculateSegments({ garageLocation, trips, router, gpsSnapshots })
    const totalShiftVehicleKm = end - start
    const manualByTrip = new Map()
    const reconciledSegments = base.segments.map(s => ({ ...s }))
    ...
    if (unclassifiedKm > EPSILON_KM) reconciledSegments.push({ id: 'SHIFT_RECONCILIATION_UNCLASSIFIED', tripId: null, label: 'SHIFT_RECONCILIATION_REMAINDER', classification: 'UNCLASSIFIED', authority: 'ODOMETER_REMAINDER', distanceKm: unclassifiedKm, method: 'ODOMETER_RECONCILIATION', confidence: 'AUTHORITATIVE_REMAINDER', roadMatchedGeometry: [] })
