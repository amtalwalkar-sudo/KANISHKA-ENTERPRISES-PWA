...
function submit() {
  ...
  // Keep the authoritative reading when one exists; otherwise use the initial manual reading.
  const startOdometer = hasAuthoritativeReading.value ? props.latestOdometer : value
  // eslint-disable-next-line vue/require-explicit-emits
  emit('start-day', startOdometer)
}
    ...
    <div class="card-body">
      <label class="odometer-field">
        ...
        <input
          ...
          min="0"
          step="1"
          :readonly="hasAuthoritativeReading"
          autocomplete="off"
          aria-label="Start odometer"
        />
      </label>
      <p v-if="hasAuthoritativeReading" class="status-notice">
        Using the latest recorded odometer reading.
      </p>
      <p v-else class="status-notice">
        Enter the vehicle's current odometer reading to begin.
      </p>
    </div>

    <div class="card-actions">
      <button type="button" class="btn btn-primary btn-large" @click="submit">
        Start Day
      </button>
    </div>
  </div>
...
.card-copy { margin: 0; opacity: 0.76; }
...
.odometer-field input:read-only { opacity: 0.8; }
.status-notice { margin: 0; font-size: 0.9rem; opacity: 0.72; }
