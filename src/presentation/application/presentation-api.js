...
function readActiveTripDraft() {
  try {
    const raw = globalThis.localStorage?.getItem(ACTIVE_TRIP_DRAFT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    return parsed?.active_trip || null;
  } catch {
    return null;
  }
}

function writeActiveTripDraft(activeTrip) {
  globalThis.localStorage?.setItem(ACTIVE_TRIP_DRAFT_KEY, JSON.stringify({ active_trip: activeTrip }));
}

function clearActiveTripDraft() {
  globalThis.localStorage?.removeItem(ACTIVE_TRIP_DRAFT_KEY);
}

function tripBusinessDate(value = new Date()) {
...
export function createKfePresentationApi({ app = application, commandActions = actions } = {}) {
  ...
  async function startTrip(payload = {}) {
    ...
    const activeTrip = {
      trip_id: payload.trip_id || `trp_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`,
      trip_type: tripType,
      shift_id: tripType === 'BUSINESS' ? (payload.shift_id || null) : null,
    ...
    try {
      const result = tripType === 'BUSINESS'
        ? await app.startBusinessTrip({
            startOdometer: startOdometerKm,
            startedAt,
            businessDate,
            shiftId: activeTrip.shift_id,
            actionMode: payload.actionMode || 'SWIPE',
            direction: payload.direction || 'RIGHT',
          })
        : await app.startPersonalTrip({
            odometer: startOdometerKm,
            prefilledOdometer: payload.prefilledOdometer ?? null,
            businessKm: payload.businessKm || 0,
            personalKm: payload.personalKm || 0,
            startedAt,
            businessDate,
            actionMode: payload.actionMode || 'SWIPE',
            direction: payload.direction || 'LEFT',
          });
      writeActiveTripDraft({ ...activeTrip, trip_id: result?.id || activeTrip.trip_id });
      return result;
    } catch (error) {
    ...
    getWorkState: async (...args) => {
      try {
        ...
        return {
          ...
          active_trip: trip,
          draft_keys_restored: trip ? [ACTIVE_TRIP_DRAFT_KEY] : [],
          state_source: trip ? 'LOCAL_STORAGE' : 'LOCAL_DB',
          screen_state: screenState,
          state_error: null,
        };
      } catch (error) {
        return { state: 'OFF_SHIFT', rehydrated: true, active_shift: null, active_trip: null, draft_keys_restored: [], state_source: 'LOCAL_DB', screen_state: 'DAY_START', state_error: 'CORRUPTED', recovery_error: String(error?.message || error) };
      }
    },
