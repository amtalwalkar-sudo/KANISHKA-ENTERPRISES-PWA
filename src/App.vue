...
const historicalSaving=ref(false);const historicalNotice=ref('');const historicalError=ref('');
...
async function navigate(path){const result=await interaction.run(async()=>router.navigate(path));if(!result.accepted)return;state.set(UI_STATES.READY);uiState.value=state.state;interaction.reset();if(path==='Status')await loadStatus();if(path==='Timeline')await loadTimeline();if(path==='Fuel')await loadFuel();}
...
async function handleHistoricalSave(payload){historicalSaving.value=true;historicalNotice.value='';historicalError.value='';try{if(payload.kind==='HISTORICAL_DAY')await application.recordHistoricalDay(payload.value);else if(payload.kind==='HISTORICAL_FUEL')await application.recordHistoricalFuel(payload.value);else throw new Error('Unknown historical entry');payload.done?.();historicalNotice.value=payload.kind==='HISTORICAL_DAY'?'Historical day saved safely.':'Historical fuel entry saved safely.';}catch(error){historicalError.value=String(error?.message||error);payload.done?.();}finally{historicalSaving.value=false;}}
async function handleResetRequest(){try{await application.resetAllData();window.location.reload();}catch(error){statusModel.value={error:String(error?.message||error)};}}
