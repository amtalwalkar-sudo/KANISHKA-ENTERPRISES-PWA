...
const PRIMARY_DESTINATIONS=['Work','Performance','Timeline'];
...
const currentDestination=computed(()=>PRIMARY_DESTINATIONS.includes(activeModule.value)?activeModule.value:'Work');
...
const headerShiftSeconds=computed(()=>workHeader.value.startedAt?Math.max(0,Math.floor((Date.now()-Date.parse(workHeader.value.startedAt))/1000)):0);
...
async function loadWorkHeader(){try{const model=await application.getWorkScreenState();workHeader.value={shiftActive:model.shift.active,startedAt:model.shift.startedAt,latestOdometer:model.latestOdometer??null};}catch{workHeader.value={shiftActive:false,startedAt:null,latestOdometer:null};}}
async function loadPerformance(){try{performanceModel.value=await application.getPerformance();}catch(error){performanceModel.value={error:String(error?.message||error)};}}
async function loadTimeline(){try{const model=await application.getTimeline(timelineHorizon.value);timelineEvents.value=model.events||[];}catch{timelineEvents.value=[];}}
async function loadFuel(){try{fuelRecords.value=await application.listFuel();}catch{fuelRecords.value=[];}}
async function navigate(path){const result=await interaction.run(async()=>router.navigate(path));if(!result.accepted)return;state.set(UI_STATES.READY);uiState.value=state.state;interaction.reset();if(path==='Performance')await loadPerformance();if(path==='Timeline')await loadTimeline();if(path==='Fuel')await loadFuel();if(path==='Work')await loadWorkHeader();}
async function changeTimelineHorizon(horizon){timelineHorizon.value=horizon;await loadTimeline();}
...
function handleBack(){router.handleBack();}function handleWorkStateChanged(){void loadWorkHeader();void loadPerformance();}function openGlobalFuel(){if(!globalFuelOpen.value)globalFuelOpen.value=true;}function closeGlobalFuel(){globalFuelOpen.value=false;void loadWorkHeader();void loadPerformance();}
...
onUnmounted(()=>{router.stop();lifecycle.stop();stopReducedMotionWatch();window.removeEventListener('kfe:work-state-changed',handleWorkStateChanged);clearInterval(workHeaderTimer);});
...
<WorkSessionView v-if="activeModule==='Work'"/><PerformanceModuleView v-else-if="activeModule==='Performance'" :online="online" :performance="performanceModel"/>
...
<AuthoritativeRecordForm v-if="historicalEditEvent" :type="historicalEditEvent.entityType==='Work Session'?'WORK_SESSION':historicalEditEvent.entityType==='Business Trip'?'BUSINESS_TRIP':historicalEditEvent.entityType==='Personal Trip'?'PERSONAL_TRIP':historicalEditEvent.entityType==='Fuel'?'FUEL':historicalEditEvent.entityType==='Revenue'?'REVENUE':historicalEditEvent.entityType==='Expense'?'EXPENSE':''" mode="EDIT" :record="historicalEditEvent" @saved="handleAuthoritativeSaved" @close="closeAuthoritativeForm" />
<nav class="kfe-bottom-nav" aria-label="Primary driver navigation"><button v-for="destination in PRIMARY_DESTINATIONS" :key="destination" class="kfe-nav-item" type="button" :aria-current="currentDestination===destination?'page':undefined" @click="navigate(destination)">{{destination}}</button></nav></div>
</template>
<style>
.kfe-global-fuel-action{position:fixed;right:14px;bottom:calc(68px + env(safe-area-inset-bottom));z-index:900}.kfe-global-fuel-action button{border:0;border-radius:999px;padding:12px 16px;font-weight:800;box-shadow:0 8px 24px rgba(0,0,0,.18);background:var(--kfe-shell-surface,#fff);color:var(--kfe-shell-text,#111);border:1px solid var(--kfe-shell-border,#ddd)}.kfe-work-session .fuel-tab-wrap{display:none}
</style>
