...
async function loadFuel(){try{fuelRecords.value=await application.listFuel();}catch(error){fuelRecords.value=[];}}
...
function openMoreItem(item){navigate(item);}function selectTimelineHorizon(horizon){timelineHorizon.value=horizon;}
function openModuleAction(action){if(action?.module==='Fuel'&&action?.action==='Fuel history'){fuelHistoryOpen.value=true;void loadFuel();}}
function returnToMore(){fuelHistoryOpen.value=false;navigate('More');}
...
function handleWorkStateChanged(){void loadWorkHeader();}
...
<VehicleModuleView v-else-if="activeModule==='Vehicle'" @save-request="handleSaveRequest" @back="returnToMore"/><DriverModuleView v-else-if="activeModule==='Driver'" @save-request="handleSaveRequest" @back="returnToMore"/><MaintenanceModuleView v-else-if="activeModule==='Maintenance'" @save-request="handleSaveRequest"/><ComplianceModuleView v-else-if="activeModule==='Compliance'" @save-request="handleSaveRequest" @back="returnToMore"/><LoanModuleView v-else-if="activeModule==='Loans'" @save-request="handleSaveRequest" @calculation-request="handleCalculationRequest" @open="openModuleAction" @back="returnToMore"/>
<MoneyModuleView v-else-if="MONEY_MODULES.includes(activeModule)" :module="activeModule" :fuel-records="fuelRecords" :fuel-history-open="fuelHistoryOpen" @save-request="handleSaveRequest" @open="openModuleAction" @fuel-history-back="fuelHistoryOpen=false" @fuel-edit="handleFuelEdit" @fuel-undo="handleFuelUndo" @back="returnToMore"/>
<KfeFinancialModuleView v-else-if="FINANCIAL_MODULES.includes(activeModule)" :module="activeModule" @open="openModuleAction" @back="returnToMore"/><KfeModuleView v-else :module="activeModule" @open="openModuleAction" @reset-request="handleResetRequest" @save-request="handleSaveRequest"/>
