...
function syncValues(source = {}) {
  ...
  for (const field of fields.value) {
    values[field.key] = source[field.key] !== undefined ? source[field.key] : field.defaultValue;
  }
  errors.value = {};
}
...
function setValue(key, value) {
  ...
  if (errors.value[key]) { const next = { ...errors.value }; delete next[key]; errors.value = next; }
}
...
.form-field input[type="checkbox"] { width: auto; justify-self: start; }
