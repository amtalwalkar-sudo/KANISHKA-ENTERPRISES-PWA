...
<template>
<div class="kfe-swipe-bar" :class="{ 'is-disabled': disabled, 'is-dual': Boolean(leftText&&rightText) }" role="button" tabindex="0" :aria-disabled="disabled" :aria-label="[leftText,rightText].filter(Boolean).join(' or ')" @pointerdown="down" @pointerup="up" @pointercancel="capturing=false" @keydown="keyboard" @click="accessibleClick">
