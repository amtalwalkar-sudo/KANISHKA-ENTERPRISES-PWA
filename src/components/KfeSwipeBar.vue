...
function keyboard(event){if(event.key!=='Enter'&&event.key!==' ')return;event.preventDefault();if(props.disabled)return;if(props.rightAction&&!props.rightDisabled)emit('swipe',props.rightAction);else if(props.leftAction&&!props.leftDisabled)emit('swipe',props.leftAction);}
function accessibleClick(event){if(event.detail!==0||props.disabled)return;if(props.rightAction&&!props.rightDisabled)emit('swipe',props.rightAction);else if(props.leftAction&&!props.leftDisabled)emit('swipe',props.leftAction);}
</script>
<template>
