  ...
  <section class="kfe-module-view" :data-module="module" :aria-labelledby="`${module}-title`">
    <template v-if="activeAction">
      <div class="kfe-module-heading">
        ...
        <p class="kfe-destination-subtitle">{{ formSpec.subtitle }}</p>
      </div>
      <KfeFormShell :draft-key="`erp:${module}`" :title="formSpec.title" :subtitle="formSpec.subtitle" @save="save">
        <template #default="{ value }">
          <KfeFormField v-for="field in formSpec.fields" :key="field.id" v-bind="field" v-model="value[field.id]" />
          <p class="kfe-form-boundary-note">Business scope is fixed. Financial calculations remain in the domain/application layer.</p>
        </template>
      </KfeFormShell>
    ...
    <template v-else>
      ...
      <section class="kfe-summary-grid" aria-label="Module summary">
      ...
      <p v-if="savedMessage" class="kfe-boundary-note" role="status">{{ savedMessage }}</p>
      <p v-if="error" class="kfe-error-note" role="alert">{{ error }}</p>
      <section class="kfe-module-section">
        <div class="kfe-section-heading"><h2>RECENT RECORDS</h2><span v-if="loading">Loading…</span></div>
        <div v-if="recent.length" class="kfe-record-list">
          <article v-for="row in recent" :key="row.id || `${row.occurredAt}-${row.amount}`" class="kfe-record-card">
            <div><strong>{{ row.description || (module === 'Revenue' ? 'Business revenue' : 'Business expense') }}</strong><span>{{ row.occurredAt ? new Date(row.occurredAt).toLocaleDateString() : '—' }}</span></div>
            <strong>{{ money(row.amount) }}</strong>
          </article>
        </div>
        <p v-else-if="!loading" class="kfe-boundary-note">No {{ module.toLowerCase() }} records have been entered yet.</p>
      </section>
    </template>
  </section>
</template>
...
<style scoped>
.kfe-module-view{padding:16px 0 32px;display:grid;gap:16px}.kfe-module-heading{display:grid;gap:6px}.kfe-summary-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.kfe-summary-grid article{padding:16px;border:1px solid var(--kfe-ui-border);border-radius:15px;background:var(--kfe-ui-surface);display:grid;gap:6px}.kfe-summary-grid span,.kfe-record-card span{font-size:.74rem;color:var(--kfe-muted-text)}.kfe-summary-grid strong{font-size:1.15rem}.kfe-section-heading{display:flex;justify-content:space-between;align-items:center}.kfe-section-heading h2{margin:0}.kfe-section-heading span{font-size:.72rem;color:var(--kfe-muted-text)}.kfe-record-list{display:grid;gap:8px}.kfe-record-card{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:13px 14px;border:1px solid var(--kfe-ui-border);border-radius:14px;background:var(--kfe-ui-surface)}.kfe-record-card div{display:grid;gap:4px;min-width:0}.kfe-record-card div strong{overflow:hidden;text-overflow:ellipsis}.kfe-record-card>strong{white-space:nowrap}@media(min-width:650px){.kfe-summary-grid{grid-template-columns:repeat(2,minmax(180px,260px))}}
</style>
