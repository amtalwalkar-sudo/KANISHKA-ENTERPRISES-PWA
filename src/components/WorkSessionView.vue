  ...
  <section class="kfe-work-session" aria-labelledby="work-title">
    ...
    <template v-else>
      ...
      <div v-if="form" class="work-form-overlay" role="dialog" aria-modal="true"><div class="work-form-card"><p class="kfe-eyebrow">{{ form === 'DAY_START' ? 'START OF DAY' : form === 'PERSONAL_START' ? 'START PERSONAL TRIP' : form === 'PERSONAL_END' ? 'END PERSONAL TRIP' : 'END SHIFT' }}</p>
        <div v-if="form === 'DAY_START'" class="work-form-fields"><label>Odometer *<input v-model="dayStartOdometer" inputmode="numeric" type="number" min="0" step="1"></label><label>Business KM *<input v-model="dayBusinessKm" inputmode="numeric" type="number" min="0" step="1"></label><label>Personal KM *<input v-model="dayPersonalKm" inputmode="numeric" type="number" min="0" step="1"></label></div>
        <div v-else-if="form === 'PERSONAL_START'" class="work-form-fields"><label>Start odometer *<input v-model="personalStartOdometer" inputmode="numeric" type="number" min="0" step="1"></label><label>Business KM *<input v-model="personalBusinessKm" inputmode="numeric" type="number" min="0" step="1"></label><label>Personal KM *<input v-model="personalPersonalKm" inputmode="numeric" type="number" min="0" step="1"></label></div>
        <div v-else-if="form === 'PERSONAL_END'" class="work-form-fields"><label>End odometer *<input v-model="personalEndOdometer" inputmode="numeric" type="number" min="0" step="1"></label><label>Toll<input v-model="personalToll" inputmode="decimal" type="number" min="0" step="0.01"></label><label>Parking<input v-model="personalParking" inputmode="decimal" type="number" min="0" step="0.01"></label></div>
        <div v-else class="work-form-fields"><label>End odometer *<input v-model="shiftEndOdometer" inputmode="numeric" type="number" min="0" step="1"></label><label>Revenue *<input v-model="shiftRevenue" inputmode="decimal" type="number" min="0" step="0.01"></label><label>Toll<input v-model="shiftToll" inputmode="decimal" type="number" min="0" step="0.01"></label><label>Parking<input v-model="shiftParking" inputmode="decimal" type="number" min="0" step="0.01"></label><label><input v-model="shiftTollIncluded" type="checkbox"> Toll included in fare</label><label><input v-model="shiftParkingIncluded" type="checkbox"> Parking included in fare</label></div>
      </div></div>
      <div v-if="endDayConfirm" class="work-form-overlay" role="dialog" aria-modal="true"><div class="work-form-card"><p class="kfe-eyebrow">END OF DAY</p><h2>End today's work day?</h2><p class="muted">This closes the operational day. You can start a new day later.</p><div class="work-form-actions"><button type="button" class="secondary-action" @click="endDayConfirm=false">Cancel</button><button type="button" class="primary-action" :disabled="busy" @click="confirmEndDay">Confirm End Day</button></div></div></div>
    </template>
  </section>
</template>
