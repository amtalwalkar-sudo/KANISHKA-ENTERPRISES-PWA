    ...
    <template v-else>
      ...
      <div v-if="form" class="work-form-overlay" role="dialog" aria-modal="true">
        <div class="work-form-card">
          ...
          <div v-else-if="form === 'SHIFT_END'" class="work-form-fields"><label>Revenue *<input v-model="shiftRevenue" inputmode="decimal" type="number" min="0" step="0.01"></label><label>End odometer *<input v-model="shiftEndOdometer" inputmode="numeric" type="number" min="0" step="1"></label><label>Toll <span class="muted">Optional</span><input v-model="shiftToll" inputmode="decimal" type="number" min="0" step="0.01"></label><fieldset><legend>Toll fare treatment</legend><label><input v-model="shiftTollIncluded" type="radio" :value="true" name="shift-toll"> Included in fare</label><label><input v-model="shiftTollIncluded" type="radio" :value="false" name="shift-toll"> Not included in fare</label></fieldset><label>Parking <span class="muted">Optional</span><input v-model="shiftParking" inputmode="decimal" type="number" min="0" step="0.01"></label><fieldset><legend>Parking fare treatment</legend><label><input v-model="shiftParkingIncluded" type="radio" :value="true" name="shift-parking"> Included in fare</label><label><input v-model="shiftParkingIncluded" type="radio" :value="false" name="shift-parking"> Not included in fare</label></fieldset><p class="muted">Revenue and end odometer are compulsory. Toll and parking are optional.</p></div>
          <button class="secondary-action" type="button" :disabled="busy" @click="closeForm">Cancel</button>
        </div>
      </div>
      <div v-if="endDayConfirm" class="work-form-overlay" role="dialog" aria-modal="true"><div class="work-form-card"><p class="kfe-eyebrow">END DAY</p><h2>Confirm day closure</h2><button class="primary-action" type="button" :disabled="busy" @click="confirmEndDay">Confirm</button><button class="secondary-action" type="button" :disabled="busy" @click="endDayConfirm = false">Cancel</button></div></div>
    </template>
  </section>
</template>
