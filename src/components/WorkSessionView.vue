...
const shiftTollNotIncluded = ref(false), shiftParkingNotIncluded = ref(false)
...
function openForm(kind) { error.value = ''; notice.value = ''; form.value = kind; ocrReviewOpen.value = false; if (kind === 'DAY_START') dayStartOdometer.value = latestOdometer.value == null ? '' : String(latestOdometer.value); if (kind === 'PERSONAL_START') personalStartOdometer.value = latestOdometer.value == null ? '' : String(latestOdometer.value); if (kind === 'PERSONAL_END') personalEndOdometer.value = ''; if (kind === 'SHIFT_END') { shiftRevenue.value = ''; shiftEndOdometer.value = ''; shiftToll.value = ''; shiftParking.value = ''; shiftTollNotIncluded.value = false; shiftParkingNotIncluded.value = false } }
...
async function dispatch(type, payload = {}) { if (busy.value) return null; busy.value = true; error.value = ''; try { const result = await actions.dispatch(createUiCommand(type,payload)); const root=document.querySelector('.work-form-card'); if(root) clearFormDraft(root); form.value = null; ocrReviewOpen.value = false; endDayConfirm.value = false; await load(); return result } catch (e) { error.value = String(e?.message || e); return null } finally { busy.value = false } }
  ...
  <section class="kfe-work-session" aria-labelledby="work-title">
    ...
    <template v-else>
      ...
      <div v-if="form" class="work-form-overlay" role="dialog" aria-modal="true"><div class="work-form-card" data-kfe-draft-form="true" :data-kfe-draft-key="`work:${form}`"><p class="kfe-eyebrow">{{ form === 'DAY_START' ? 'START OF DAY' : form === 'PERSONAL_START' ? 'START PERSONAL TRIP' : form === 'PERSONAL_END' ? 'END PERSONAL TRIP' : 'END SHIFT' }}</p>
        ...
        <div v-else class="work-form-fields shift-end-fields"><label>Revenue *<input v-model="shiftRevenue" inputmode="decimal" type="number" min="0" step="0.01"></label><label>End odometer *<input v-model="shiftEndOdometer" inputmode="numeric" type="number" min="0" step="1"></label><div class="optional-row"><label>Toll <span class="optional">Optional</span><input v-model="shiftToll" inputmode="decimal" type="number" min="0" step="0.01"></label><label>Parking <span class="optional">Optional</span><input v-model="shiftParking" inputmode="decimal" type="number" min="0" step="0.01"></label></div><div class="fare-checks"><label><input v-model="shiftTollNotIncluded" type="checkbox"> Toll was not included in fare</label><label><input v-model="shiftParkingNotIncluded" type="checkbox"> Parking was not included in fare</label></div><button class="ocr-review-entry" type="button" :class="{ 'is-empty': unreviewedOcrRideCount === 0 }" :disabled="unreviewedOcrRideCount === 0" :aria-label="`Unreviewed OCR rides: ${unreviewedOcrRideCount}`" @click="openOcrReview"><span>Unreviewed OCR rides</span><strong>{{ unreviewedOcrRideCount }}</strong><span v-if="unreviewedOcrRideCount > 0" class="ocr-review-hint">Review rides →</span></button><p class="muted">Revenue and end odometer are required. Toll and parking are optional. Tick the box only when that charge was not included in the fare. Then use the Close Shift swipe.</p></div>
        <button class="secondary-action" type="button" :disabled="busy" @click="closeForm">Cancel</button>
      </div></div>
      <div v-if="ocrReviewOpen" class="work-form-overlay ocr-review-overlay" role="dialog" aria-modal="true" aria-labelledby="ocr-review-title"><div class="work-form-card ocr-review-card"><p class="kfe-eyebrow">OCR RIDE REVIEW</p><h2 id="ocr-review-title">Unreviewed rides</h2><p class="muted">{{ unreviewedOcrRideCount }} ride{{ unreviewedOcrRideCount === 1 ? '' : 's' }} need review.</p><div class="ocr-review-list"><article v-for="(ride,index) in unreviewedOcrRides" :key="ride?.id || index" class="ocr-review-item"><div><strong>{{ ride?.id || `Ride ${index + 1}` }}</strong><span v-if="ride?.amount != null">₹{{ ride.amount }}</span></div><p v-if="ride?.date || ride?.time">{{ [ride?.date, ride?.time].filter(Boolean).join(' · ') }}</p><p v-if="ride?.pickup || ride?.dropoff">{{ [ride?.pickup, ride?.dropoff].filter(Boolean).join(' → ') }}</p></article></div><button class="secondary-action" type="button" @click="ocrReviewOpen = false">Back to Close Shift</button></div></div>
...
<style scoped>
.ocr-review-entry{width:100%;display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:8px;margin-top:2px;padding:10px 12px;border:1px solid var(--kfe-border,#d8dde5);border-radius:10px;background:var(--kfe-surface,#fff);text-align:left;font:inherit;color:inherit}
.ocr-review-entry:not(:disabled){cursor:pointer}
.ocr-review-entry strong{font-size:1rem}
.ocr-review-entry.is-empty{opacity:.72;cursor:default}
.ocr-review-hint{font-size:.78rem;font-weight:700;white-space:nowrap}
.ocr-review-overlay{z-index:20}
.ocr-review-card{max-height:min(72vh,620px);overflow:auto}
.ocr-review-list{display:grid;gap:7px;margin:4px 0 10px}
.ocr-review-item{padding:9px 10px;border:1px solid var(--kfe-border,#d8dde5);border-radius:9px;background:var(--kfe-surface,#fff)}
.ocr-review-item>div{display:flex;justify-content:space-between;gap:8px}
.ocr-review-item p{margin:3px 0 0;font-size:.8rem;color:var(--kfe-muted,#68707d)}
@media(max-width:600px){.ocr-review-entry{grid-template-columns:1fr auto}.ocr-review-hint{grid-column:1/-1}.ocr-review-card{max-height:70vh}}
</style>
