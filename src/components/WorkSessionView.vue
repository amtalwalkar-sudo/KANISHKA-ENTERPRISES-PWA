...
async function load() { loading.value = true; try { model.value = await application.getWorkScreenState(); if (screenState.value === 'SHIFT_WAITING' || screenState.value === 'BUSINESS_TRIP' || screenState.value === 'PERSONAL_TRIP' || screenState.value === 'DAY_START' || screenState.value === 'DAY_ENDED') workflowStateOverride.value = null; workSummary.value = model.value?.state === 'DAY_ENDED' ? await application.getWorkSummary() : null; publishDriverState() } catch (e) { error.value = String(e?.message || e) } finally { loading.value = false } }
  ...
  <section class="kfe-work-session" :class="[activeStateThemeClass, `theme-${theme.toLowerCase()}`]" aria-labelledby="work-title" :data-kfe-rehydrated="rehydrated ? 'true' : undefined" :data-kfe-state-source="stateSource" :data-kfe-state-error="stateError || undefined">
    ...
    <template v-else>
      ...
      <main class="work-main">
        ...
        <section v-else-if="displayScreenState === 'BUSINESS_TRIP'" class="work-state-panel trip-panel business-trip-card"><p class="kfe-eyebrow">BUSINESS TRIP</p><div class="trip-timer">{{ duration(tripElapsed) }}</div><p class="muted">Trip active</p></section>
        <section v-else-if="displayScreenState === 'PERSONAL_TRIP'" class="work-state-panel trip-panel personal-trip-card"><p class="kfe-eyebrow">PERSONAL TRIP</p><div class="trip-timer">{{ duration(tripElapsed) }}</div><p class="muted">Personal trip active</p></section>
      </main>
      <WorkBreakControl v-if="activeShift" />
      <WorkSessionActions :form="form" :screen-state="displayScreenState" :busy="busy" :latest-odometer="latestOdometer" :day-allocation-valid="dayAllocationValid" :personal-allocation-valid="personalAllocationValid" :personal-end-valid="personalEndValid" :shift-end-valid="shiftEndValid" :shift-revenue-valid="shiftRevenueValid" @swipe="handleSwipe" @state-authority="onStateAuthority" />
