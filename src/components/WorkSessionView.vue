...
const activeStateThemeClass = computed(() => { if (form.value === 'SHIFT_END') return 'state-close'; if (form.value === 'PERSONAL_START' || form.value === 'PERSONAL_END' || screenState.value === 'PERSONAL_TRIP') return 'state-personal'; if (screenState.value === 'SHIFT_WAITING' || screenState.value === 'BUSINESS_TRIP') return 'state-business'; return 'state-standby' })
...
async function load() { loading.value = true; try { model.value = await application.getWorkScreenState(); workSummary.value = model.value?.state === 'DAY_ENDED' ? await application.getWorkSummary() : null; publishDriverState() } catch (e) { error.value = String(e?.message || e) } finally { loading.value = false } }
async function loadWorkState() { try { const restored = await application.getWorkState(); rehydrated.value = restored?.rehydrated === true; stateSource.value = restored?.state_source || 'LOCAL_DB'; stateError.value = restored?.state_error || null; publishRehydratedState(restored?.state || 'OFF_SHIFT') } catch (e) { rehydrated.value = true; stateSource.value = 'LOCAL_DB'; stateError.value = 'CORRUPTED'; publishRehydratedState('OFF_SHIFT') } }
...
onMounted(async () => { installFormDraftRecovery(); timer = setInterval(() => { now.value = Date.now() }, 1000); window.addEventListener('keydown', onKey); await loadWorkState(); await load() })
...
<template>
  <section class="kfe-work-session" :class="[activeStateThemeClass, `theme-${theme.toLowerCase()}`]" aria-labelledby="work-title" :data-kfe-rehydrated="rehydrated ? 'true' : undefined" :data-kfe-state-source="stateSource" :data-kfe-state-error="stateError || undefined">
    ...
    <template v-else>
      <header class="work-header"><div><p class="kfe-eyebrow">Operational cockpit</p><h1 id="work-title">Work</h1><p>Today · <span class="state-badge">{{ screenState.replaceAll('_', ' ') }}</span></p></div><div class="work-header-status"><button type="button" class="theme-toggle-btn" @click="cycleTheme" aria-label="Toggle Night Mode Theme">[{{ theme }}]</button><span class="work-shift-mini">SHIFT {{ activeShift ? duration(shiftElapsed) : '—' }}</span></div></header>
      <div v-if="error" class="work-error" role="alert">{{ error }}</div><div v-if="notice" class="work-notice" role="status">✓ {{ notice }}</div>
      <main class="work-main">
        <section v-if="screenState === 'DAY_START'" class="work-state-panel day-start-panel start-of-day-panel"><div class="welcome-card" role="banner"><span class="kfe-eyebrow">WELCOME</span><h2>Hello, Welcome to Kanishka Enterprises</h2></div><p class="kfe-eyebrow">START OF DAY</p><h2>Ready for operation</h2><p class="muted">{{ latestOdometer == null ? 'Enter the odometer to establish the first authoritative reading.' : 'Your latest authoritative odometer is ready for the day-start flow.' }}</p></section>
        <section v-else-if="screenState === 'DAY_ENDED'" class="work-state-panel day-start-panel day-ended-panel"><p class="kfe-eyebrow">DAY ENDED</p><h2>Shift summary</h2><div class="shift-summary-grid" aria-label="Completed day shift summary"><article v-for="card in shiftSummaryCards" :key="card.label" class="shift-summary-card"><span>{{ card.label }}</span><strong>{{ card.value }}</strong></article></div></section>
