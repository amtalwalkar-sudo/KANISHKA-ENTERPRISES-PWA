...
.kfe-work-session {
  ...
  /* State Tokens */
  --state-standby: #374151;
  --state-business: #1D4ED8;
  --state-personal: #10B981;
  --state-close: #DC2626;
  --accent-amber: #F59E0B;
  --amber-bg: #FEF3C7;
  --amber-border: #FCD34D;
...
/* NIGHT THEME TONES (MATTE HIGH CONTRAST) */
.kfe-work-session.theme-night {
  --bg-canvas: #111827;
  --bg-surface: #1F2937;
  --bg-header: #030712;
  --text-main: #F9FAFB;
  ...
  /* Desaturated Night Accents to avoid glare */
  --state-business: #3B82F6;
  --state-personal: #059669;
  --state-close: #EF4444;
  --amber-bg: #451A03;
  --amber-border: #92400E;
  --amber-text: #FDE68A;
...
.personal-trip-card {
  border-left: 6px solid var(--state-personal);
}

.business-trip-card {
  border-left: 6px solid var(--state-business);
}
...
.work-form-fields input[type="number"],
.work-form-fields input[type="text"] {
  width: 100%;
  height: 48px;
...
.work-form-fields input:focus {
  outline: 2px solid var(--state-business);
  border-color: transparent;
}

.input-invalid {
  border-color: var(--state-close) !important;
  outline: 1px solid var(--state-close);
}
