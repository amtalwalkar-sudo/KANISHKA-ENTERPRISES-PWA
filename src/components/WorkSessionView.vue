...
const shiftRevenueValid=computed(()=>Number.isFinite(Number(shiftRevenue.value))&&Number(shiftRevenue.value)>=0&&String(shiftRevenue.value).trim()!=='');
...
function setError(v){error.value=v;}function clearError(){error.value='';}
function openForm(kind){clearError();form.value=kind;if(kind==='DAY_START')dayStartOdometer.value=latestOdometer.value==null?'':String(latestOdometer.value);if(kind==='PERSONAL_START')personalStartOdometer.value=latestOdometer.value==null?'':String(latestOdometer.value);if(kind==='PERSONAL_END')personalEndOdometer.value=latestOdometer.value==null?'':String(latestOdometer.value);if(kind==='SHIFT_END'){shiftEndOdometer.value='';shiftRevenue.value='';shiftToll.value='';shiftParking.value='';shiftTollIncluded.value=false;shiftParkingIncluded.value=false;window.__KFE_SHIFT_REVENUE_PAISE=null;}}
function closeForm(){if(!busy.value){form.value=null;clearError();}}
async function load(){loading.value=true;clearError();try{model.value=await application.getWorkScreenState();summary.value=await application.workSummary();}catch(e){setError(String(e?.message||e));}finally{loading.value=false;}}
async function dispatch(type,payload){if(busy.value)return null;busy.value=true;clearError();try{const r=await actions.dispatch(createUiCommand(type,payload));form.value=null;endDayConfirm.value=false;await load();return r;}catch(e){setError(String(e?.message||e));return null;}finally{busy.value=false;}}
function announce(text,action){notice.value=text;lastAction.value=action||null;clearTimeout(announce.timer);announce.timer=setTimeout(()=>{notice.value='';lastAction.value=null;},5000);}
async function confirmDayStart(){const n=Number(dayStartOdometer.value);if(!Number.isInteger(n)||n<0)return setError('Enter the day-start odometer.');if(!dayDiff.value.valid)return setError('Odometer cannot decrease.');if(!dayAllocationValid.value)return setError(`Allocate exactly ${dayDiff.value.difference} km between business and personal.`);const r=await dispatch('START_DAY',{odometer:n,prefilledOdometer:latestOdometer.value,businessKm:dayDiff.value.required?Number(dayBusinessKm.value):0,personalKm:dayDiff.value.required?Number(dayPersonalKm.value):0,actionMode:'SWIPE',direction:'RIGHT'});if(r)announce('Day started safely');}
async function confirmPersonalStart(){const n=Number(personalStartOdometer.value);if(!Number.isInteger(n)||n<0)return setError('Enter the personal-trip start odometer.');if(!personalDiff.value.valid)return setError('Odometer cannot decrease.');if(!personalAllocationValid.value)return setError(`Allocate exactly ${personalDiff.value.difference} km between business and personal.`);const r=await dispatch('START_PERSONAL_TRIP',{odometer:n,prefilledOdometer:latestOdometer.value,businessKm:personalDiff.value.required?Number(personalBusinessKm.value):0,personalKm:personalDiff.value.required?Number(personalPersonalKm.value):0,actionMode:'SWIPE',direction:'LEFT'});if(r)announce('Personal trip started safely');}
async function startShift(){const r=await dispatch('START_SHIFT',{actionMode:'SWIPE',direction:'RIGHT'});if(r)announce('Shift started safely');}
async function startBusinessTrip(){const r=await dispatch('START_TRIP',{actionMode:'SWIPE',direction:'RIGHT'});if(r)announce('Business trip started safely');}
async function endBusinessTrip(){const r=await dispatch('END_TRIP',{id:activeTrip.value?.id,actionMode:'SWIPE',direction:'RIGHT'});if(r)announce('Business trip saved safely');}
async function confirmPersonalEnd(){const n=Number(personalEndOdometer.value);if(!Number.isInteger(n)||n<0)return setError('End odometer is compulsory.');if(!personalEndValid.value)return setError('End odometer cannot be below the trip start odometer.');const r=await dispatch('END_PERSONAL_TRIP',{id:activeTrip.value?.id,endOdometer:n,tollPaise:Math.round(Math.max(0,Number(personalToll.value)||0)*100),parkingPaise:Math.round(Math.max(0,Number(personalParking.value)||0)*100),actionMode:'SWIPE',direction:'RIGHT'});if(r)announce('Personal trip saved safely');}
async function confirmShiftEnd(){if(!shiftRevenueValid.value)return setError('Revenue is compulsory to close the business shift.');const n=Number(shiftEndOdometer.value);if(!Number.isInteger(n)||n<0)return setError('End odometer is compulsory.');if(!shiftEndValid.value)return setError('End odometer cannot be below the shift start odometer.');window.__KFE_SHIFT_REVENUE_PAISE=Math.round(Number(shiftRevenue.value)*100);const r=await dispatch('END_SHIFT',{id:activeShift.value?.id,endOdometer:n,tollPaise:Math.round(Math.max(0,Number(shiftToll.value)||0)*100),parkingPaise:Math.round(Math.max(0,Number(shiftParking.value)||0)*100),tollIncludedInFare:shiftTollIncluded.value,parkingIncludedInFare:shiftParkingIncluded.value,actionMode:'SWIPE',direction:'LEFT'});if(!r)window.__KFE_SHIFT_REVENUE_PAISE=null;else announce('Shift closed safely');}
async function confirmEndDay(){if(!canEndDay.value)return;const r=await dispatch('END_DAY',{actionMode:'BUTTON',direction:null});if(r)announce('Day ended safely');}
...
function requestEndDay(){if(canEndDay.value){clearError();endDayConfirm.value=true;}}
function cancelEndDay(){if(!busy.value){endDayConfirm.value=false;clearError();}}
...
<section class="kfe-work-session" aria-labelledby="work-title">
  ...
  <template v-else>
    ...
    <main class="work-main">
      ...
      <section v-else-if="screenState==='BUSINESS_TRIP'||screenState==='PERSONAL_TRIP'" class="work-state-panel trip-panel"><p class="kfe-eyebrow">{{screenState==='BUSINESS_TRIP'?'BUSINESS TRIP':'PERSONAL TRIP'}}</p><div class="trip-timer">{{duration(tripElapsed)}}</div><p class="muted">{{screenState==='BUSINESS_TRIP'?'Trip active':'Personal trip active'}}</p></section>
    </main>
    <div class="bottom-action" aria-label="Work action">
      ...
      <KfeSwipeBar v-else-if="form==='DAY_START'" right-label="START DAY" right-action="CONFIRM_DAY_START" :disabled="busy||!dayAllocationValid" @swipe="()=>confirmDayStart()" />
      <KfeSwipeBar v-else-if="form==='PERSONAL_START'" right-label="START PERSONAL TRIP" right-action="CONFIRM_PERSONAL_START" :disabled="busy||!personalAllocationValid" @swipe="()=>confirmPersonalStart()" />
      ...
      <KfeSwipeBar v-else-if="form==='SHIFT_END'" left-label="CANCEL" right-label="CLOSE SHIFT" right-action="CLOSE_SHIFT" :disabled="busy||!shiftEndValid||!shiftRevenueValid" @swipe="handleSwipe" />
    </div>
