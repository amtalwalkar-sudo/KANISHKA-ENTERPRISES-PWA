...
const shiftTollIncluded = ref(false)
const shiftParkingIncluded = ref(false)
...
function openForm(kind) {
  ...
  if (kind === 'SHIFT_END') {
    ...
    shiftToll.value = ''
    shiftParking.value = ''
    shiftTollIncluded.value = false
    shiftParkingIncluded.value = false
  }
}
...
async function confirmDayStart() {
  ...
  await dispatch('START_DAY', { odometer, businessKm: dayDiff.value.required ? Number(dayBusinessKm.value) : 0, personalKm: dayDiff.value.required ? Number(dayPersonalKm.value) : 0, actionMode: 'SWIPE', direction: 'RIGHT' })
}
...
async function confirmPersonalStart() {
  ...
  await dispatch('START_PERSONAL_TRIP', { odometer, businessKm: personalDiff.value.required ? Number(personalBusinessKm.value) : 0, personalKm: personalDiff.value.required ? Number(personalPersonalKm.value) : 0, actionMode: 'SWIPE', direction: 'LEFT' })
}
    ...
    <template v-else>
      <header class="work-header">
        <div>
          ...
          <p>Today · {{ screenState.replaceAll('_', ' ') }}</p>
        </div>
        ...
        <div class="work-form-card">
          ...
          <div v-if="form === 'DAY_START'" class="work-form-fields">
            ...
            <div v-if="dayDiff.required" class="allocation-block">
              <p>Allocate {{ dayDiff.difference }} km</p>
          ...
          <div v-else-if="form === 'PERSONAL_START'" class="work-form-fields">
            ...
            <div v-if="personalDiff.required" class="allocation-block">
              <p>Allocate {{ personalDiff.difference }} km</p>
      ...
      <div v-if="endDayConfirm" class="work-form-overlay" role="dialog" aria-modal="true">
        <div class="work-form-card">
          ...
          <button class="primary-action" type="button" :disabled="busy" @click="confirmEndDay">Confirm</button>
          <button class="secondary-action" type="button" :disabled="busy" @click="endDayConfirm = false">Cancel</button>
        </div>
      </div>
    </template>
  </section>
