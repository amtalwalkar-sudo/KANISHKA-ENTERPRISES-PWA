...
import { projectTimeline, buildTimelineJourneyModel, summarizeOutstationJourneys, summarizeTimelineActivity, summarizeFuelActivity } from '../../js/ui/timeline.js';
...
function selectFuelMode(value){fuelMode.value=value;}
function fuelList(){if(fuelMode.value==='MUMBAI')return fuelSummary.value.mumbai;if(fuelMode.value==='OUTSTATION')return fuelSummary.value.outstation;if(fuelMode.value==='UNKNOWN')return fuelSummary.value.unknown;return [...fuelSummary.value.mumbai,...fuelSummary.value.outstation,...fuelSummary.value.unknown].sort((a,b)=>Date.parse(a.occurredAt)-Date.parse(b.occurredAt));}
function fuelLocationLabel(event){return event.locationName||'Location unavailable';}
function fuelAmount(event){return event.amount!==null&&event.amount!==undefined?`₹ ${Number(event.amount).toFixed(2)}`:'Amount unavailable';}
...
const fuelSummary=computed(()=>summarizeFuelActivity(projectedEvents.value));
 ...
 <div class="kfe-timeline-view" :data-horizon="horizon">
  ...
  <template v-else>
   <div v-if="displayEvents.length" class="kfe-longterm">
    ...
    <section class="kfe-fuel-summary" aria-label="Fuel refuel summary">
     <header class="kfe-fuel-summary-heading"><div><span>⛽ FUEL</span><h2>Refuels by location</h2></div><strong>{{fuelSummary.total}} refuels</strong></header>
     <div class="kfe-fuel-summary-grid">
      <button type="button" class="kfe-fuel-summary-card" :class="{'is-selected':fuelMode==='ALL'}" @click="selectFuelMode('ALL')"><span>Total refuels</span><strong>{{fuelSummary.total}}</strong><small>All recorded fuel events</small></button>
      <button type="button" class="kfe-fuel-summary-card" :class="{'is-selected':fuelMode==='MUMBAI'}" @click="selectFuelMode('MUMBAI')"><span>Mumbai</span><strong>{{fuelSummary.mumbaiCount}}</strong><small>Tap for Mumbai fuel details</small></button>
      <button type="button" class="kfe-fuel-summary-card" :class="{'is-selected':fuelMode==='OUTSTATION'}" @click="selectFuelMode('OUTSTATION')"><span>Outstation</span><strong>{{fuelSummary.outstationCount}}</strong><small>Tap for outstation fuel details</small></button>
     </div>
     <div v-if="fuelSummary.unknownCount" class="kfe-fuel-unclassified"><span>{{fuelSummary.unknownCount}} refuel{{fuelSummary.unknownCount===1?'':'s'}} without a classified location</span><button type="button" @click="selectFuelMode('UNKNOWN')">View unclassified</button></div>

     <section v-if="fuelMode!=='ALL' || fuelSummary.total" class="kfe-fuel-detail-card">
      <header><div><span>{{fuelMode==='MUMBAI'?'MUMBAI FUEL':fuelMode==='OUTSTATION'?'OUTSTATION FUEL':fuelMode==='UNKNOWN'?'UNCLASSIFIED FUEL':'ALL FUEL'}}</span><h3>{{fuelList().length}} refuels</h3></div><button v-if="fuelMode!=='ALL'" type="button" class="kfe-fuel-back" @click="selectFuelMode('ALL')">All refuels</button></header>
      <div v-if="fuelList().length" class="kfe-fuel-details-list">
       <article v-for="(fuel,index) in fuelList()" :key="fuel.id || `fuel:${fuel.occurredAt}:${index}`" class="kfe-fuel-detail-item">
        <div class="kfe-fuel-detail-icon" aria-hidden="true">⛽</div>
        <div class="kfe-fuel-detail-main"><strong>{{dateLabel(fuel.occurredAt)}} · {{timeLabel(fuel.occurredAt)}}</strong><span>📍 {{fuelLocationLabel(fuel)}}</span><div class="kfe-fuel-detail-meta"><span>{{fuelAmount(fuel)}}</span><span v-if="fuel.quantity!==null&&fuel.quantity!==undefined">{{fuel.quantity}} {{fuel.unit||'L'}}</span><span v-if="fuel.fuelType">{{fuel.fuelType}}</span><span v-if="fuel.vehicle">{{fuel.vehicle}}</span></div><small v-if="fuel.reference">Ref: {{fuel.reference}}</small></div>
       </article>
      </div>
      <KfeStatePanel v-else state="empty" title="No refuels in this category" message="Fuel events will appear here when authoritative fuel records with a location are available." />
     </section>
    </section>
...
<style scoped>
.kfe-fuel-summary{margin:18px 0 22px;padding:16px;border:1px solid var(--kfe-border,#dfe4ea);border-radius:16px;background:var(--kfe-surface,#fff)}
.kfe-fuel-summary-heading{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px}.kfe-fuel-summary-heading span{font-size:.72rem;font-weight:800;letter-spacing:.08em}.kfe-fuel-summary-heading h2{margin:3px 0 0;font-size:1.05rem}.kfe-fuel-summary-heading>strong{font-size:.95rem;white-space:nowrap}
.kfe-fuel-summary-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.kfe-fuel-summary-card{appearance:none;border:1px solid var(--kfe-border,#dfe4ea);border-radius:12px;background:transparent;text-align:left;padding:12px;cursor:pointer}.kfe-fuel-summary-card.is-selected{border-color:currentColor}.kfe-fuel-summary-card span,.kfe-fuel-summary-card small{display:block}.kfe-fuel-summary-card span{font-size:.78rem}.kfe-fuel-summary-card strong{display:block;font-size:1.35rem;margin:4px 0}.kfe-fuel-summary-card small{font-size:.7rem;opacity:.72}
.kfe-fuel-unclassified{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-top:10px;font-size:.76rem}.kfe-fuel-unclassified button,.kfe-fuel-back{border:0;background:none;text-decoration:underline;cursor:pointer;padding:4px}
.kfe-fuel-detail-card{margin-top:12px;padding:14px;border-radius:12px;background:color-mix(in srgb,currentColor 4%,transparent)}.kfe-fuel-detail-card>header{display:flex;justify-content:space-between;align-items:center;gap:12px}.kfe-fuel-detail-card header span{font-size:.7rem;font-weight:800;letter-spacing:.06em}.kfe-fuel-detail-card h3{margin:3px 0 0;font-size:1rem}.kfe-fuel-details-list{display:grid;gap:8px;margin-top:10px}.kfe-fuel-detail-item{display:flex;gap:10px;padding:10px;border:1px solid var(--kfe-border,#dfe4ea);border-radius:10px;background:var(--kfe-surface,#fff)}.kfe-fuel-detail-icon{font-size:1.15rem}.kfe-fuel-detail-main{min-width:0;display:grid;gap:3px}.kfe-fuel-detail-main>strong{font-size:.82rem}.kfe-fuel-detail-main>span{font-size:.76rem}.kfe-fuel-detail-meta{display:flex;flex-wrap:wrap;gap:8px;font-size:.74rem}.kfe-fuel-detail-main small{font-size:.68rem;opacity:.7}
@media (max-width:640px){.kfe-fuel-summary-grid{grid-template-columns:1fr}.kfe-fuel-summary-heading{align-items:flex-start;flex-direction:column}.kfe-fuel-unclassified{align-items:flex-start;flex-direction:column}}
</style>
