...
const formRef=ref(null),busy=ref(false),error=ref(''),success=ref(''),gpsStatus=ref('Waiting for location…'),lastPosition=ref(null),latestFuel=ref(null),loadingLatest=ref(false);
...
const latestQuantityKg=computed(()=>{const r=latestFuel.value;if(!r)return null;if(r.quantity_kg!=null)return Number(r.quantity_kg);const a=Number(r.amount_paise),p=Number(r.price_per_kg);return Number.isFinite(a)&&Number.isFinite(p)&&p>0?a/100/p:null;});
...
function startEditLatest(){if(!latestFuel.value||busy.value)return;emit('close');setTimeout(()=>emit('saved',{__openEditFuel:latestFuel.value}),0);}
