...
import { clearFormDraft } from '../../js/ui/form-drafts.js';
...
function cancel(){if(busy.value)return;if(formRef.value?.dataset.kfeDraftPresent==='true'&&!isEdit.value){const ok=globalThis.confirm?.('Discard this unsaved draft?');if(ok===false)return;}clearFormDraft(formRef.value);emit('close');}
