...
const pendingShiftOdometer = ref(null)
...
function onSwipe(action) {
  ...
  if (action === 'START_PERSONAL_TRIP') {
    ...
    pendingShiftOdometer.value = authorityState.value === 'SHIFT_WAITING' ? null : pendingShiftOdometer.value
...
function submitStartShift(odometer) {
  startShiftOpen.value = false
  pendingShiftOdometer.value = Number.isInteger(Number(odometer)) ? Number(odometer) : null
...
watch(() => props.screenState, state => {
  ...
  if (state === 'SHIFT_WAITING') {
    startShiftOpen.value = false
    pendingShiftOdometer.value = null
  }
}, { immediate: true })
</script>
...
<template>
  <div class="bottom-action" aria-label="Work action" data-state-authority="swipe-bar">
    ...
    <KfeSwipeBar v-else-if="!form && authorityState === 'SHIFT_WAITING'" left-label="START PERSONAL TRIP" right-label="START SHIFT" left-action="START_PERSONAL_TRIP" right-action="START_SHIFT" :disabled="busy" @swipe="onSwipe" />
