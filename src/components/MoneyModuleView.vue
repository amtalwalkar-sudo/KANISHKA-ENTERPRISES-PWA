...
function openAction(item){
 if(item==='Fuel history'){emit('open','Fuel history');return;}
 if(['Expense history','Revenue history'].includes(item)){detail.value=item;return;}
 if(formSpec.value|| (props.module==='Fuel'&&item==='Add fuel')||(props.module==='Expenses'&&['Add expense','Toll','Parking'].includes(item))||(props.module==='Revenue'&&item==='Enter today’s revenue'))activeAction.value=item;else emit('open',item);
}
...
<button class="kfe-secondary-action kfe-back-action" type="button" @click="detail?closeDetail():activeAction?closeAction():emit('back')">‹ {{detail?definition.title:activeAction?definition.title:'More'}}</button>
<template v-if="detail">
<p class="kfe-eyebrow">Money</p><h1 id="money-title">{{detail}}</h1><p class="kfe-destination-subtitle">Authoritative records supplied by the application read model.</p>
<article class="kfe-detail-card"><strong>{{detail}}</strong><p>History is a read-only presentation surface. Record creation and financial calculations remain outside this component.</p></article>
</template>
<template v-else>
