...
export const ShiftTripRepository = {
  ...
  async completeShift(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const mutations = tx.objectStore('pending_mutations')
      const request = shifts.get(data.id)

      request.onsuccess = () => {
        const shift = request.result
        if (!shift) { reject(new Error('Active shift not found.')); return }
        if (shift.status !== 'ACTIVE') { reject(new Error('Shift is not active.')); return }

        const now = new Date().toISOString()
        try {
          for (const correction of data.trips || []) {
            if (!correction?.id) continue
            const tripRequest = trips.get(correction.id)
            tripRequest.onsuccess = () => {
              const trip = tripRequest.result
              if (!trip || trip.shiftId !== shift.id || trip.status !== 'COMPLETED') return
            ...
            tripRequest.onerror = () => { throw tripRequest.error || new Error('Trip correction failed.') }
          }
      ...
      request.onerror = () => reject(request.error || new Error('Active shift lookup failed.'))
      tx.oncomplete = () => resolve(true)
