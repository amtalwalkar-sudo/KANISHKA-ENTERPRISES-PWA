...
export const ShiftTripRepository = {
  ...
  async completeShift(data) {
    ...
    return new Promise((resolve, reject) => {
      ...
      shiftRequest.onerror = () => reject(shiftRequest.error || new Error('Active shift lookup failed.')); tx.oncomplete = () => resolve(true); tx.onerror = () => reject(tx.error || new Error('Atomic shift completion failed.')); tx.onabort = () => reject(tx.error || new Error('Atomic shift completion aborted.'))
    })
  },
