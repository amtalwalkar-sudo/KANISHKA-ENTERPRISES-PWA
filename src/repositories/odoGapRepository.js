...
export const OdoGapRepository = {
  ...
  async getAll() { const db = await initializeCanonicalStorage(); return new Promise((resolve, reject) => { const tx = db.transaction('odoGaps', 'readonly'); const req = tx.objectStore('odoGaps').getAll(); req.onsuccess = () => resolve(req.result || []); req.onerror = () => reject(req.error || new Error('Failed to read odometer gaps.')) }) },
