...
export const MovementArtifactRepository = {
  async create({ shiftId, generatedAt, segments = [], gpsTracePoints = 0, routing = {} }) {
    const db = await initializeCanonicalStorage(); const now = new Date().toISOString(); const record = { id: generateUUID(), entityType: 'SHIFT_MOVEMENT_ARTIFACT', entityId: shiftId, shiftId, generatedAt: generatedAt || now, createdAt: now, gpsTracePoints: Number(gpsTracePoints) || 0, routing: routing || {}, segments: Array.isArray(segments) ? segments : [] }
