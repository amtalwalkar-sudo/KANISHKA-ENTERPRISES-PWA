...
const goOffline = async () => {
  ...
  const result = await store.endShift({ closingOdometer: closingOdo.value, toll: toll.value, parking: parking.value, tollParkingRevenueTreatment: tollTreatment.value, trips })
  if (!result.ok) return fail(result.reason)
...
<template>
  <div class="cockpit">
    ...
    <section v-if="!store.isOnline" class="card gate"><h2>Start Shift</h2><p class="muted">The Online toggle opens this gate. The shift starts only after the odometer check succeeds.</p><label>Current odometer (km)<input v-model="startOdo" type="number" min="0" inputmode="decimal"></label><div v-if="gap.valid && gap.gapKm > 0" class="gap"><strong>Odometer gap: {{ gap.gapKm }} km</strong><p>Allocate the complete gap between Personal and Dead KM.</p><label>Personal KM<input v-model="gapPersonal" type="number" min="0" step="0.1"></label><label>Dead KM<input v-model="gapDead" type="number" min="0" step="0.1"></label><div class="two"><label>Personal toll (optional)<input v-model="personalToll" type="number" min="0" step="0.01"></label><label>Personal parking (optional)<input v-model="personalParking" type="number" min="0" step="0.01"></label></div><small>Allocated {{ allocatedGap }} / {{ gap.gapKm }} km</small></div><p v-else-if="gap.valid" class="muted">No odometer gap.</p><p v-else-if="startOdo" class="error-text">{{ gap.reason }}</p></section>
