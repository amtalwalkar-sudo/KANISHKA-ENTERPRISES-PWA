...
const tripTimer = computed(() => {
  ...
  return [Math.floor(seconds / 3600), Math.floor((seconds % 3600) / 60), seconds % 60].map((n, i) => i === 0 ? String(n).padStart(2, '0') : String(n).padStart(2, '0')).join(':')
})
...
<template>
  <div class="cockpit">
    ...
    <section v-if="!store.isOnline" class="card gate">
      <h2>Go Online</h2><p>Start Shift</p>
      <label>Current odometer (km)<input v-model="startOdo" type="number" min="0" inputmode="decimal"></label>
      <div v-if="gap.valid && gap.gapKm > 0" class="gap"><strong>Odometer gap: {{ gap.gapKm }} km</strong><p>Allocate the complete gap before the Shift starts.</p><label>Personal KM<input v-model="gapPersonal" type="number" min="0" step="0.1"></label><label>Dead KM<input v-model="gapDead" type="number" min="0" step="0.1"></label><small>Allocated {{ allocatedGap }} / {{ gap.gapKm }} km</small></div>
      <p v-else-if="gap.valid" class="muted">No odometer gap.</p><p v-else-if="startOdo" class="error-text">{{ gap.reason }}</p>
      <button class="primary" @click="goOnline">ONLINE</button>
    </section>

    <section v-else-if="!store.isTripActive" class="card gate">
      <h2>Start Trip</h2><div class="current-operator"><span>Operator for this trip</span><strong>{{ selectedOperator || store.defaultOperator }}</strong></div><p class="muted">Check this before every ride. The last selected operator carries forward.</p>
      ...
      <button class="primary" @click="startTrip">START TRIP</button>
    </section>

    <section v-if="store.isTripActive" class="card trip">
      <small>TRIP ACTIVE</small><h2>{{ store.trip.operator }}</h2><div class="timer">{{ tripTimer }}</div><button class="danger" @click="endTrip">END TRIP</button><button class="quiet" @click="cancelTrip">Cancel accidental trip</button>
    </section>
    ...
    <section v-if="store.isOnline && !store.isTripActive" class="card gate">
      ...
      <button class="danger" @click="goOffline">OFFLINE</button>
    </section>
  </div>
</template>

<style scoped>
.cockpit{max-width:600px;margin:auto;padding:16px 16px 110px;color:#0f172a}.hero{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px}.hero small{font-size:.65rem;font-weight:900;color:#64748b;letter-spacing:.12em}.hero h1{margin:2px 0;font-size:1.45rem}.hero b{padding:7px 10px;border-radius:18px;font-size:.7rem}.on{background:#dcfce7;color:#166534}.off{background:#e2e8f0;color:#475569}.card{background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:16px;margin-bottom:14px;box-shadow:0 1px 2px rgba(15,23,42,.05)}.state{display:flex;justify-content:space-between}.state div{display:flex;flex-direction:column;gap:3px}.state span,.muted{font-size:.75rem;color:#64748b}.state strong{font-size:.9rem}.gate h2{margin:0 0 5px;font-size:1rem}.gate label{display:block;font-size:.76rem;font-weight:800;margin:11px 0}.gate input,.gate select,.review input,.review select{width:100%;box-sizing:border-box;padding:10px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;margin-top:5px;font:inherit}.primary,.danger,.secondary,.quiet{width:100%;padding:13px;border:0;border-radius:9px;font-weight:900;margin-top:12px}.primary{background:#111827;color:#fff}.danger{background:#dc2626;color:#fff}.secondary{background:#e2e8f0;color:#1e293b}.quiet{background:transparent;color:#64748b}.gap{background:#fff7ed;border:1px solid #fdba74;border-radius:10px;padding:12px}.gap p{font-size:.75rem;color:#92400e}.current-operator{display:flex;justify-content:space-between;padding:12px;background:#f1f5f9;border-radius:10px}.operators{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.operators button{padding:11px;border:1px solid #cbd5e1;border-radius:9px;background:#fff;font-weight:800}.operators button.selected{border:2px solid #111827}.trip{text-align:center}.trip h2{margin:7px 0}.timer{font:700 2rem ui-monospace,SFMono-Regular,Menlo,monospace;margin:14px 0}.two{display:grid;grid-template-columns:1fr 1fr;gap:10px}.review{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-top:8px}.review input,.review select{font-size:.7rem;padding:8px}.message,.error{padding:10px 12px;border-radius:9px;margin-bottom:12px;font-size:.8rem}.message{background:#dcfce7;color:#166534}.error,.error-text{color:#991b1b}.error{background:#fee2e2}@media(max-width:380px){.two,.review{grid-template-columns:1fr}}
</style>
