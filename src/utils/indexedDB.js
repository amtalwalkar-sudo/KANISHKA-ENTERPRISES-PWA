...
export const openCanonicalDB = () => new Promise((resolve, reject) => {
  ...
  request.onupgradeneeded = (e) => {
    ...
    if (!db.objectStoreNames.contains('shifts')) { const store = db.createObjectStore('shifts', { keyPath: 'id' }); store.createIndex('shiftEndAt', 'shiftEndAt', { unique: false }) } else { const store = tx.objectStore('shifts'); if (store.indexNames.contains('legacyId')) store.deleteIndex('legacyId') }
    if (!db.objectStoreNames.contains('fuel_logs')) { const store = db.createObjectStore('fuel_logs', { keyPath: 'id' }); store.createIndex('createdAt', 'createdAt', { unique: false }) } else { const store = tx.objectStore('fuel_logs'); if (store.indexNames.contains('legacyId')) store.deleteIndex('legacyId') }
    ...
    if (!db.objectStoreNames.contains('financial_inputs')) { const store = db.createObjectStore('financial_inputs', { keyPath: 'id' }); store.createIndex('kind', 'kind', { unique: false }); store.createIndex('status', 'status', { unique: false }); store.createIndex('effectiveFrom', 'effectiveFrom', { unique: false }); store.createIndex('effectiveTo', 'effectiveTo', { unique: false }) }
    if (!db.objectStoreNames.contains('admin_records')) { const store = db.createObjectStore('admin_records', { keyPath: 'recordKey' }); store.createIndex('formKey', 'formKey', { unique: false }); store.createIndex('updatedAt', 'updatedAt', { unique: false }) }
  }
...
export const initializeCanonicalStorage = async () => { if (isInitialized && dbInstance) return dbInstance; if (initializationPromise) return initializationPromise; initializationPromise = (async () => { const db = await openCanonicalDB(); isInitialized = true; return db })(); try { return await initializationPromise } catch (error) { isInitialized = false; throw error } finally { initializationPromise = null } }

export const saveCompletedShift = async (data) => {
  const db = await openCanonicalDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction('shifts', 'readwrite'); const store = tx.objectStore('shifts'); const now = new Date().toISOString()
    const record = { id: data.id || generateUUID(), startOdometer: Number(data.startOdometer) || 0, endOdometer: Number(data.endOdometer) || 0, totalDistance: Number(data.totalDistance) || 0, revenue: Number(data.revenue) || 0, shiftStartAt: data.shiftStartAt || now, shiftEndAt: data.shiftEndAt || now, createdAt: data.createdAt || now, updatedAt: now }
    try { store.put(record) } catch (error) { reject(error); return }
    tx.oncomplete = () => resolve(record); tx.onerror = () => reject(tx.error || new Error('Shift persistence failed.')); tx.onabort = () => reject(tx.error || new Error('Shift persistence aborted.'))
  })
}
