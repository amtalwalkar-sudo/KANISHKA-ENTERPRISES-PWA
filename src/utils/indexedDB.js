...
const migrateLegacyDataStrict = async (canonicalDb) => {
  ...
  await new Promise((resolve, reject) => {
    ...
    const processIfReady = () => {
      ...
      const existingLegacyShiftIds = new Set(existingShifts.filter((record) => record.legacySource === LEGACY_SOURCE && record.legacyId !== null && record.legacyId !== undefined).map((record) => String(record.legacyId)))
      const existingLegacyFuelIds = new Set(existingFuelLogs.filter((record) => record.legacySource === LEGACY_SOURCE && record.legacyId !== null && record.legacyId !== undefined).map((record) => String(record.legacyId)))
