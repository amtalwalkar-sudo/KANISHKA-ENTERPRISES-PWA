...
export const openCanonicalDB = () => new Promise((resolve, reject) => {
  ...
  request.onupgradeneeded = (e) => {
    ...
    if (!db.objectStoreNames.contains('admin_records')) {
      const store = db.createObjectStore('admin_records', { keyPath: 'id' })
      store.createIndex('entityType', 'entityType', { unique: false })
  ...
  request.onsuccess = () => {
    ...
    dbInstance.onversionchange = () => {
      dbInstance.close(); dbInstance = null; isInitialized = false; initializationPromise = null
    }
    resolve(dbInstance)
  }
...
export const getAllCompletedShifts = async () => {
  const db = await openCanonicalDB()
  return new Promise((resolve, reject) => { const tx = db.transaction('shifts', 'readonly'); const req = tx.objectStore('shifts').getAll(); req.onsuccess = () => resolve(req.result || []); req.onerror = () => reject(req.error || new Error('Failed to read completed shifts.')) })
}

export const getLastOdometer = async () => {
  const shifts = await getAllCompletedShifts(); if (!shifts.length) return 0
  const sorted = [...shifts].sort((a, b) => new Date(b.shiftEndAt || b.createdAt).getTime() - new Date(a.shiftEndAt || a.createdAt).getTime()); return Number(sorted[0].endOdometer) || 0
}

export const saveFuelLog = async (data) => {
  const db = await openCanonicalDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction('fuel_logs', 'readwrite'); const store = tx.objectStore('fuel_logs'); const now = new Date().toISOString()
    const record = { id: data.id || generateUUID(), odometer: Number(data.odometer) || 0, pricePerKg: Number(data.pricePerKg) || 0, amount: Number(data.amount) || 0, kg: Number(data.kg) || 0, createdAt: data.createdAt || now, updatedAt: now }
    try { store.put(record) } catch (error) { reject(error); return }
    tx.oncomplete = () => resolve(record); tx.onerror = () => reject(tx.error || new Error('Fuel log persistence failed.')); tx.onabort = () => reject(tx.error || new Error('Fuel log persistence aborted.'))
  })
}
