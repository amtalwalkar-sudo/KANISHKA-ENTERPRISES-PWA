...
assert.equal(m.actualProfit, -270)
...
assert.ok(Number.isFinite(m.loanProvision))
assert.ok(Number.isFinite(m.actualLoanPaid))
assert.equal(m.authority.deadKm, 'VEHICLE_KM_MINUS_BUSINESS_KM')
console.log('Performance contract passed: canonical sources, dead KM, actual maintenance, loan history, provisions and break-even are wired.')
