...
const valid = { format: BACKUP_FORMAT, formatVersion: BACKUP_FORMAT_VERSION, source: { dbName: 'kanishka_kfe_canonical_db', dbVersion: 8 }, exportedAt: '2026-09-16T00:00:00.000Z', stores: emptyStores }

assert.equal(CANONICAL_BACKUP_STORES.length, 19)
assert.deepEqual(BackupService.getBackupSummary(valid), { exportedAt: valid.exportedAt, totalRecords: 0, counts: emptyStores })
...
assert.throws(() => BackupService.validateBackup({ ...valid, formatVersion: 2 }), /Unsupported backup format version/)
