...
export function inspectAdminImpact(kind, values, existing) {
  ...
  if (start && end && new Date(end) < new Date(start)) {
    warnings.push('This correction makes the recorded end time earlier than the start time. KFE will retain the correction and dependent timeline calculations may change.')
  }
  if (values.openingOdometer !== '' && values.closingOdometer !== '' && Number(values.closingOdometer) < Number(values.openingOdometer)) {
    warnings.push('This correction moves the closing odometer below the opening odometer. Vehicle KM, dead KM and performance results may change.')
  }
  if (values.amountPaid !== '' && values.amount !== '' && Number(values.amountPaid) > Number(values.amount)) {
    warnings.push('Amount paid exceeds the recorded amount. This historical correction may affect payment and financial reporting.')
  }
  if (values.complianceAmountPaid !== '' && values.complianceAmount !== '' && Number(values.complianceAmountPaid) > Number(values.complianceAmount)) {
    warnings.push('Compliance amount paid exceeds the recorded amount. This historical correction may affect financial reporting.')
  }
  if (values.maintenanceAmountPaid !== '' && values.maintenanceAmount !== '' && Number(values.maintenanceAmountPaid) > Number(values.maintenanceAmount)) {
    warnings.push('Maintenance amount paid exceeds the recorded amount. This historical correction may affect financial reporting.')
  }
