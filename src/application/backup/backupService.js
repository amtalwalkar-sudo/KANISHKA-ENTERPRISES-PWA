...
const openLocalBackupDB = () => new Promise((resolve, reject) => { const request = indexedDB.open(LOCAL_BACKUP_DB_NAME, LOCAL_BACKUP_DB_VERSION); request.onupgradeneeded = event => { const db = event.target.result; if (!db.objectStoreNames.contains('snapshots')) db.createObjectStore('snapshots', { keyPath: 'id' }) }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error || new Error('Local backup storage could not be opened.')) })
...
const readCanonicalSnapshot = async () => {
  ...
  return new Promise((resolve, reject) => {
    ...
    tx.onerror = () => fail(tx.error || new Error('Backup snapshot transaction failed.')); tx.onabort = () => { if (!settled) { settled = true; reject(tx.error || new Error('Backup snapshot transaction aborted.')) } }
  })
}
...
export const validateBackup = input => {
  ...
  const expected = new Set(backup.formatVersion === 1 ? LEGACY_BACKUP_STORES : CANONICAL_BACKUP_STORES); const actual = Object.keys(backup.stores)
  ...
  for (const storeName of expected) { const records = backup.stores[storeName]; if (!Array.isArray(records)) throw new Error(`Backup store ${storeName} must be an array.`); const ids = new Set(); for (const record of records) { if (!record || typeof record !== 'object' || Array.isArray(record) || typeof record.id !== 'string' || !record.id.trim()) throw new Error(`Backup store ${storeName} contains an invalid record.`); if (ids.has(record.id)) throw new Error(`Backup store ${storeName} contains duplicate id ${record.id}.`); ids.add(record.id) } }
...
export const createBackup = async () => validateBackup({ format: BACKUP_FORMAT, formatVersion: BACKUP_FORMAT_VERSION, source: { dbName: 'kanishka_kfe_canonical_db', dbVersion: 9 }, exportedAt: new Date().toISOString(), stores: await readCanonicalSnapshot() })
