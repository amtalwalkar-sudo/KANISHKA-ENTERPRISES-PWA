...
export const CANONICAL_BACKUP_STORES = Object.freeze([
  'shifts',
  'fuel_logs',
  'odoGaps',
  'pending_mutations',
...
const readStore = (tx, storeName) => new Promise((resolve, reject) => {
  const request = tx.objectStore(storeName).getAll()
  request.onsuccess = () => resolve(request.result || [])
  request.onerror = () => reject(request.error || new Error(`Backup read failed for ${storeName}.`))
})

const readCanonicalSnapshot = async () => {
  const db = await initializeCanonicalStorage()
  return new Promise((resolve, reject) => {
    const tx = db.transaction(CANONICAL_BACKUP_STORES, 'readonly')
    const result = {}
    Promise.all(CANONICAL_BACKUP_STORES.map(async storeName => {
      result[storeName] = await readStore(tx, storeName)
    })).then(() => {
      tx.oncomplete = () => resolve(result)
      tx.onerror = () => reject(tx.error || new Error('Backup snapshot transaction failed.'))
      tx.onabort = () => reject(tx.error || new Error('Backup snapshot transaction aborted.'))
    }).catch(reject)
  })
}
...
export const validateBackup = input => {
  ...
  if (typeof input === 'string') {
    try { backup = JSON.parse(input) } catch (_) { throw new Error('Backup file is not valid JSON.') }
  }
  ...
  const expected = new Set(CANONICAL_BACKUP_STORES)
  ...
  for (const storeName of CANONICAL_BACKUP_STORES) {
    ...
    for (const record of records) {
      if (!record || typeof record !== 'object' || Array.isArray(record) || typeof record.id !== 'string' || !record.id.trim()) throw new Error(`Backup store ${storeName} contains an invalid record.`)
      if (ids.has(record.id)) throw new Error(`Backup store ${storeName} contains duplicate id ${record.id}.`)
      ids.add(record.id)
    }
  }
  return backup
...
export const createBackup = async () => {
  const stores = await readCanonicalSnapshot()
  const backup = {
    format: BACKUP_FORMAT,
    formatVersion: BACKUP_FORMAT_VERSION,
    source: { dbName: 'kanishka_kfe_canonical_db', dbVersion: 8 },
...
export const getBackupSummary = backup => {
  ...
  const counts = Object.fromEntries(CANONICAL_BACKUP_STORES.map(store => [store, valid.stores[store].length]))
  return { exportedAt: valid.exportedAt, totalRecords: Object.values(counts).reduce((sum, count) => sum + count, 0), counts }
}

export const saveLocalBackup = async backup => {
  const valid = validateBackup(backup)
  const db = await openLocalBackupDB()
  return new Promise((resolve, reject) => {
    const tx = db.transaction('snapshots', 'readwrite')
    tx.objectStore('snapshots').put({ id: LOCAL_BACKUP_ID, savedAt: new Date().toISOString(), backup: valid })
  ...
  await new Promise((resolve, reject) => {
    ...
    try {
      for (const storeName of CANONICAL_BACKUP_STORES) {
        ...
        store.clear()
        for (const record of backup.stores[storeName]) store.add(structuredClone(record))
      }
    } catch (error) {
...
export const downloadBackup = async (backup, filename = null) => {
  const valid = validateBackup(backup)
  const blob = new Blob([serializeBackup(valid)], { type: 'application/json' })
  const url = URL.createObjectURL(blob)
  const anchor = document.createElement('a')
  anchor.href = url
  anchor.download = filename || `kfe-backup-${valid.exportedAt.replace(/[:.]/g, '-')}.json`
...
export const requestLocalBackupCheckpoint = () => {
  ...
  checkpointTimer = setTimeout(async () => {
    ...
    if (checkpointRunning || !checkpointRequested) return
    checkpointRequested = false
    checkpointRunning = true
    try { await saveLocalBackup(await createBackup()) } catch (error) { console.warn('KFE local backup checkpoint failed:', error) } finally { checkpointRunning = false }
  }, 250)
}

export const maybeDailyLocalBackup = async () => {
  const current = await getLocalBackup()
  if (!current || Date.now() - Date.parse(current.savedAt) >= 24 * 60 * 60 * 1000) {
    const backup = await createBackup()
    await saveLocalBackup(backup)
    return getBackupSummary(backup)
  }
  return getBackupSummary(current.backup)
}
...
export const BackupService = Object.freeze({
  ...
  downloadBackup,
  readBackupFile,
  requestLocalBackupCheckpoint,
  maybeDailyLocalBackup,
})
