...
export const AdminService={
 ...
 async remove(formKey,id){const definition=definitionFor(formKey);if(!id)throw new Error('Admin record id is required.');for(const [form,field] of refs[formKey]??[]){const linked=await AdminRepository.list(form);if(linked.some(r=>r.values?.[field]===id))throw new Error(`Cannot delete this ${definition.title}: related ${definitionFor(form).title} records still reference it.`)}const removed=await AdminRepository.remove(formKey,id);checkpoint();return removed},
}
