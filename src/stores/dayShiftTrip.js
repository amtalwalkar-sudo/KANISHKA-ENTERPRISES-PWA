...
export const useDayShiftTripStore = defineStore('dayShiftTrip', () => {
  ...
  const startShift = async odo => {
    if (!isDayOnline.value) { await startDay() }
  ...
  const endShift = async (odo, revenue, garageLocation = null) => {
    ...
    if (garageLocation) await GpsSnapshotRepository.create({ entityType: 'SHIFT', entityId: id, event: 'SHIFT_END', location: garageLocation, periodic: false })
    const trips = await DayShiftTripRepository.getCompletedTripsForShift(id); const gpsSnapshots = await GpsSnapshotRepository.getForEntity(id)
    ...
    try { await MovementArtifactRepository.create({ shiftId: id, generatedAt: new Date().toISOString(), segments: reconciliation.segments, gpsTracePoints: reconciliation.gpsTracePoints, routing: { provenance: reconciliation.routingProvenance, geometry: reconciliation.roadMatchedGeometry, traceGeometry: reconciliation.traceGeometry } }) } catch (error) { alert(`Movement artifact could not be persisted: ${error.message}`); return false }
    stopShiftGps(); await DayShiftTripRepository.endShift({ id, endOdometer: v.values.endOdometer, revenue: v.values.revenue, businessKm: reconciliation.businessMilesKm, deadKm: reconciliation.deadMilesKm, unclassifiedKm: reconciliation.unclassifiedKm, movementReconciliation: reconciliation, movementReconciliationStatus: reconciliation.reconciliationStatus, revenueReconciliation: revenueResult }); await TripNotificationService.clear(); movementReconciliation.value = reconciliation; revenueReconciliation.value = revenueResult; await refresh(); return true
  }
  ...
  return { day, shift, trip, interShiftGap, reconciliationTrips, movementReconciliation, revenueReconciliation, initialized, isDayOnline, isShiftActive, isTripActive, headerShiftStatus, headerTripStatus, initialize, refresh, configureRoutingEngine, startDay, endDay, startShift, captureShiftEndLocation, allocateInterShiftGap, endShift, startTrip, endTrip, recordMissedTrip, openUberReconciliation, saveUberReconciliation, calculateDeadMiles }
})
