...
### Continuous GPS / Trip Measurement Rule

KFE may use the Capacitor/native infrastructure boundary for continuous background GPS collection while the relevant operational capability is active and the platform permits background location execution.

GPS is a **continuous measurement and evidence layer**, not a competing authoritative odometer authority.

For business trips:

- The driver explicitly performs only **Start Trip** and **End Trip** actions.
- Trip start timestamp is captured automatically.
- Trip end timestamp is captured automatically.
- GPS position and movement telemetry are captured automatically.
- GPS-derived trip distance is captured automatically.
- GPS-derived trip duration is reconstructed automatically from timestamps.
- The driver should not have to manually enter trip start/end odometer values during normal trip operation.

The authoritative vehicle-distance boundary remains the manual odometer entered at **Shift Start** and **Shift End**.

Therefore:

`Authoritative Vehicle KM = Shift End Manual Odometer − Shift Start Manual Odometer`

GPS measurements may be used to reconstruct, validate, explain, and reconcile movement, but they must never silently overwrite the authoritative manual odometer.

Where GPS-derived movement and manual authoritative movement differ, the manual authoritative movement remains the source of truth. The resulting movement must then be reconciled into the existing attribution model:

`Authoritative Vehicle Movement → Validated Business Ride KM + Personal KM + Dead / Unallocated KM`

A GPS/manual discrepancy must not simply disappear or be silently corrected. GPS error, missing telemetry, or segmentation error must not automatically be treated as genuine dead mileage; the reconciliation process determines the appropriate allocation.

### Optional Driver Trip KM / Revenue Entry

The driver may optionally enter individual trip KM and trip revenue when completing a trip.

This does not create a second mileage authority. The trip may retain:

- GPS measured KM
- Driver-confirmed KM
- Validated / accepted Trip KM
- GPS timestamps and movement evidence

The validated Trip KM is the value consumed by downstream business movement calculations.

Driver-confirmed individual trip KM can improve business-ride attribution and reduce the effect of GPS measurement/segmentation errors on dead-mile calculation.

For example:

`Authoritative Vehicle KM − Validated Business Ride KM − Validated Personal KM = Dead / Unallocated KM`

Thus GPS remains continuously useful even when the driver supplies individual trip KM. GPS provides automatic telemetry/evidence while driver-confirmed trip KM can provide stronger operational attribution.

This preserves the core rule:

> **One continuous vehicle movement record; one authoritative manual odometer boundary; GPS as automatic measurement/evidence; optional driver-confirmed trip KM and revenue; no duplicate mileage authority.**
...
> **GPS is measurement/evidence, not a competing authoritative odometer.**

> **Manual shift-start/end odometer remains authoritative.**
