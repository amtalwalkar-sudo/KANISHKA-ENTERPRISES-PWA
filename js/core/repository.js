...
export function createConfigurationRepository(storeName){
  ...
  return Object.freeze({
    ...
    async update(existing,changes={}){const record=createConfigurationSchema({data:{...existing,...changes},meta:{id:existing.id,user_id:existing.user_id,created_at:existing.created_at,updated_at:new Date().toISOString(),synced:false,is_deleted:existing.is_deleted},effective_from:changes.effective_from??existing.effective_from,effective_to:changes.effective_to??existing.effective_to,version:changes.version??existing.version});await write(storeName,record);return clone(record);},
...
export function createRepository({initial={}}={}){
  ...
  async function save(next){memory=clone(next);const record=createRecord({value:memory},{id:KEY});await write('state',record);return clone(memory);}
