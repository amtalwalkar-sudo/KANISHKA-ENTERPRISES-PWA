...
import {readMeta,writeMeta,readSafetyMeta,writeSafetyMeta,readPayload,writePayload,readDeviceKey,writeDeviceKey} from './backup-storage.js';
...
async function derivePasswordKey(passphrase,salt){
  const material=await webCrypto().subtle.importKey('raw',textEncoder.encode(String(passphrase||'')),'PBKDF2',false,['deriveKey']);
  return webCrypto().subtle.deriveKey({name:'PBKDF2',salt,iterations:PBKDF2_ITERATIONS,hash:'SHA-256'},material,AES,false,['encrypt','decrypt']);
}
async function deviceKey(){
  const existing=await readDeviceKey();
  if(existing?.key)return existing.key;
  const key=await webCrypto().subtle.generateKey(AES,false,['encrypt','decrypt']);
...
async function encodeDocument(document,key,mode){
  const raw=utf8(canonicalize(document));
  const checksum=await sha256Hex(raw);
  const packed=await gzip(raw);
  const encrypted=await encrypt(packed.bytes,key);
  return {checksum,compression:packed.compression,iv:bytesToBase64(encrypted.iv),ciphertext:bytesToBase64(encrypted.ciphertext),encryptionMode:mode,sizeBytes:encrypted.ciphertext.byteLength};
}
...
export function createBackupEngine({repository,provider=null}={}){
  if(!repository)throw new TypeError('KFE repository is required');
  if(provider)assertBackupProvider(provider);
  let datasetIdPromise=null;let refreshPromise=Promise.resolve();let started=false;let unsubscribe=()=>{};
  ...
  async function refreshLocal(reason='mutation'){
    refreshPromise=refreshPromise.then(async()=>{const snapshot=await repository.exportSnapshot();const pkg=await buildPackage(snapshot,'device');const now=new Date().toISOString();await writePayload({package:pkg,updatedAt:now,reason});const previous=await readMeta();await writeMeta({datasetId:pkg.manifest.datasetId,createdAt:previous?.createdAt||pkg.manifest.createdAt,lastSuccessfulBackupAt:now,lastReason:reason,recordCount:pkg.manifest.recordCount,sizeBytes:pkg.ciphertext.length});return pkg;});
    return refreshPromise;
  }
  async function start(){if(started)return;started=true;await requestPersistentStorage();unsubscribe=repository.subscribeMutations?.(event=>{if(event?.store==='idempotency'||event?.type==='idempotency')return;void refreshLocal('data-change').catch(()=>{});})||(()=>{});const meta=await readMeta();const payload=await readPayload();if(!meta?.lastSuccessfulBackupAt||!payload?.package)await refreshLocal('startup');return getStatus();}
  ...
  async function restorePackage(pkg,passphrase){
    validateBackupPackage(pkg);
    if(pkg.encryption.mode==='passphrase'&&String(passphrase||'').length<8)throw new RangeError('KFE backup passphrase is required');
    await refreshLocal('pre-restore-safety');
    const current=await readPayload();const currentMeta=await readMeta();
    if(current?.package)await writePayload({...current,updatedAt:new Date().toISOString()},'safety');
    if(currentMeta)await writeSafetyMeta({...currentMeta,safetyCreatedAt:new Date().toISOString()});
    const key=pkg.encryption.mode==='device'?await deviceKey():await derivePasswordKey(passphrase,base64ToBytes(pkg.encryption.salt));
    const snapshot=await decodeDocument(pkg,key);assertRepositorySnapshot(snapshot);
    await repository.importSnapshot(snapshot);
    await refreshLocal('post-restore');
    return getStatus();
  }
  ...
  async function saveCloud(pkg,metadata={}){if(!provider)throw new Error('No backup provider configured');const target=createBackupProviderContract(provider);return target.put({id:pkg.manifest.createdAt,createdAt:pkg.manifest.createdAt,package:pkg,metadata});}
  ...
  function stop(){unsubscribe();started=false;}
