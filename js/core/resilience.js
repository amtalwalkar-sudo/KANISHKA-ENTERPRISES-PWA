...
export async function initializeResilience({sendOutbox=async()=>{},dispatch=null}={}){
  sendOutboxFn=sendOutbox;
  dispatchFn=typeof dispatch==='function'?dispatch:null;
  ...
  void (dispatchFn?dispatchFn():flushOutbox(sendOutboxFn));
}

export async function enqueueNetworkMutation(payload){await queueOutbox(payload);if(navigator.onLine)void (dispatchFn?dispatchFn():flushOutbox(sendOutboxFn));}
export function broadcast(type,payload){channel?.postMessage({type,payload,at:Date.now()});}
export function closeResilience(){cleanupFns.forEach(fn=>fn());cleanupFns=[];channel?.close();channel=null;dispatchFn=null;}
