// Generic idempotency contract. No business semantics.
export function createOperationId(){return crypto.randomUUID();}
export function assertOperationId(value){if(typeof value!=='string'||!value)throw new TypeError('operationId is required');return value;}
export async function withIdempotency(repository,operationId,operation){
  ...
  await repository.saveIdempotency?.({id:operationId,result,created_at:new Date().toISOString()});
  return result;
}
