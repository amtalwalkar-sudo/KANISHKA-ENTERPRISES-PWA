...
export function fixedExpensePerBusinessKm(fixedConfigs,businessKm,at){const cfg=effectiveFixedExpenses(fixedConfigs,at);if(!cfg.length||businessKm<=0)return result(null,DATA.INSUFFICIENT_DATA,cfg.map(c=>c.id),at);const monthly=cfg.reduce((a,c)=>a+paise(c.monthly_amount_paise),0);return result(Math.round(monthly/businessKm),DATA.ACTUAL,cfg.map(c=>c.id),at);}
