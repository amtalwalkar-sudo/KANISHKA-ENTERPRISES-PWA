...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    if(!STORES.vehicles||STORES.vehicles.keyPath!=='id')throw new Error('Canonical vehicles store is missing or has the wrong keyPath');

    const repository=createVehicleRepository();
    const record={id:`vehicle-browser-round-trip-${crypto.randomUUID()}`,marker:'real-indexeddb'};
    const updated={...record,marker:'real-indexeddb-updated'};
    const contains=async expected=>{
    ...
    return {database:DB_NAME,version:DB_VERSION,store:'vehicles',keyPath:STORES.vehicles.keyPath,operations:['create/get','create/list','update/get','update/list','remove/get','remove/list'],result:'PASS'};
  });
