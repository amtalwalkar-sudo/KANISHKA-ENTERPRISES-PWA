...
try{
  ...
  const shiftState=page.locator('.work-state');
  await shiftState.waitFor({state:'visible'});
  await page.waitForFunction(()=>document.querySelector('.work-state')?.textContent?.trim()==='SHIFT ACTIVE');
  await assert.equal((await shiftState.textContent()).trim(),'SHIFT ACTIVE');
  await page.getByLabel('Shift end odometer').fill('180');
  await page.getByRole('button',{name:'End shift',exact:true}).click();
  const dayState=page.locator('.work-state');
  await page.waitForFunction(()=>document.querySelector('.work-state')?.textContent?.trim()==='DAY ACTIVE');
  await assert.equal((await dayState.textContent()).trim(),'DAY ACTIVE');
