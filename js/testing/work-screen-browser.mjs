...
try{
  ...
  // Personal Trip is a direct swipe action. No start form is expected.
  await swipe(page.locator('.kfe-swipe-bar'),'LEFT');
  await waitState('PERSONAL TRIP');
  assert.equal(await page.getByText('Personal trip',{exact:true}).first().isVisible(),true);

  // End personal trip uses the fixed bottom swipe zone; its required end odometer is captured by the close form.
  ...
  // End Day is a button, not a swipe/form. Confirmation protects against accidental taps.
  ...
  await page.getByRole('button',{name:/Confirm End Day|End Day/i}).click();
  ...
  console.log('PASS direct personal-trip swipe and fixed swipe action slot');
