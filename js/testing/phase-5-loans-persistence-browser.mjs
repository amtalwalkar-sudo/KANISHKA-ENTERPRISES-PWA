...
try{
 ...
 try{const context=await browser.newContext();try{const page=await context.newPage();await withTimeout(page.goto(`http://127.0.0.1:${port}/index.html`,{waitUntil:'domcontentloaded'}),'page.goto');const result=await withTimeout(page.evaluate(async()=>{
   ...
   if(DB_VERSION!==6)throw new Error(`Unexpected DB version: ${DB_VERSION}`);
