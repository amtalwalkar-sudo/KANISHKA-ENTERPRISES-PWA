...
assert.throws(()=>calculateWorkSession({...shift,scope:'PERSONAL'}),/Personal trips are separate from business shifts/);

const businessTrip={id:'trip-business',scope:'BUSINESS',start_at:'2026-08-28T23:10:00+05:30',end_at:'2026-08-29T01:30:00+05:30'};
assert.equal(calculateTrip(businessTrip,shift).value.tripKm,null);
assert.equal(calculateTrip(businessTrip,shift).value.businessDate,'2026-08-28');
assert.equal(validateTripLifecycle(businessTrip,shift),true);
...
const personal=calculateTrip(personalTrip,null);
