...
async function workState(selector) {
...
async function swipeStartDay() {
  ...
  if (!trackBox || !thumbBox) {
    throw new Error('Failed to resolve SwipeBar bounding boxes for native drag');
  }
  const startX = thumbBox.x + thumbBox.width / 2;
  const startY = thumbBox.y + thumbBox.height / 2;
  const endX = trackBox.x + trackBox.width * 0.90;
  await page.mouse.move(startX, startY);
  await page.mouse.down();
  await page.mouse.move(endX, startY, {steps: 10});
  await page.mouse.up();
  await page.waitForSelector('[role="dialog"][aria-labelledby="start-day-modal-title"]', {
    state: 'visible',
    timeout: 10000
  });
}
