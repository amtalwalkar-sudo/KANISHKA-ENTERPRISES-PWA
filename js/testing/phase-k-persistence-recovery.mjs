...
import {DB_NAME,DB_VERSION,STORE_NAMES,STORES,openKfeDb,read,write,all,remove,bufferCrash} from '../core/hardened-db.js';
...
class FakeObjectStore {
  ...
  put(value){const r=new FakeRequest(); queueMicrotask(()=>{try{const key=value?.id;if(key==null)throw new Error('id required');this.map.set(key,structuredClone(value));r.result=key;r.onsuccess?.();this.tx.pendingDone();}catch(e){r.error=e;this.tx.abort(e);r.onerror?.();}});return r;}
  get(key){const r=new FakeRequest();queueMicrotask(()=>{r.result=this.map.has(key)?structuredClone(this.map.get(key)):undefined;r.onsuccess?.();this.tx.pendingDone();});return r;}
  getAll(){const r=new FakeRequest();queueMicrotask(()=>{r.result=[...this.map.values()].map(structuredClone);r.onsuccess?.();this.tx.pendingDone();});return r;}
  delete(key){const r=new FakeRequest();queueMicrotask(()=>{this.map.delete(key);r.result=undefined;r.onsuccess?.();this.tx.pendingDone();});return r;}
  clear(){const r=new FakeRequest();queueMicrotask(()=>{this.map.clear();r.result=undefined;r.onsuccess?.();this.tx.pendingDone();});return r;}
}
class FakeTransaction {
  constructor(db,names){this.db=db;this.names=names;this.error=null;this.oncomplete=null;this.onerror=null;this.onabort=null;this.aborted=false;this.pending=0;this.finished=false;}
  ...
  abort(error=new Error('aborted')){if(this.aborted||this.finished)return;this.aborted=true;this.error=error;queueMicrotask(()=>this.onabort?.());}
}
class FakeDb {
