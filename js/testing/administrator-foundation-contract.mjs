...
assert.equal(vehicle.current_odometer,82450);assert.equal((await app.listOdometerHistory(vehicle.id))[0].source,'ACQUISITION');assert.equal((await app.listVehicleLifecycleHistory(vehicle.id))[0].event_type,'ACQUISITION');
...
await app.sellVehicle(vehicle.id,{sale_date:'2026-10-01',sale_odometer:83000,sale_amount:350000});assert.equal((await app.getVehicle(vehicle.id)).sale_amount_paise,35000000);const history=await app.listVehicleLifecycleHistory(vehicle.id);assert.deepEqual(history.map(x=>x.event_type),['ACQUISITION','SALE']);assert.equal(history[1].amount_paise,35000000);assert.equal((await app.listVehicleDisposals()).some(x=>x.vehicle_id===vehicle.id&&x.amount_paise===35000000),true);
