...
const port=4175;
const server=spawn('npm',['run','preview','--','--host','127.0.0.1','--port',String(port)],{detached:true,stdio:['ignore','pipe','pipe']});
let output='';
  ...
  try{
    ...
    try{
      ...
      const result=await withTimeout(page.evaluate(async()=>{
        const {createExpenseRepository}=await import('/js/application/expense-repository.js');
        ...
        if(!STORES.expense_records||STORES.expense_records.keyPath!=='id')throw new Error('Canonical expense_records store is missing or has the wrong keyPath');
        const repository=createExpenseRepository();
        const record={id:`expense-browser-round-trip-${crypto.randomUUID()}`,marker:'real-indexeddb'};
        ...
        return {database:DB_NAME,version:DB_VERSION,store:'expense_records',keyPath:STORES.expense_records.keyPath,result:'PASS'};
      }),'Expenses IndexedDB round-trip');
      console.log(`PHASE_5_EXPENSES_REAL_INDEXEDDB_PERSISTENCE=${result.result}`);
