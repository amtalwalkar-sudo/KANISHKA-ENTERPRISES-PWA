...
function parseArgs(raw,event){
  if(!raw) return [];
  const parts=[];
  let token='', quote=null, depth=0;
  for(const ch of raw){
    if(quote){ token+=ch; if(ch===quote) quote=null; continue; }
    if(ch==='"' || ch==="'"){ quote=ch; token+=ch; continue; }
    if(ch==='(' || ch==='[' || ch==='{') depth++;
    if(ch===')' || ch===']' || ch==='}') depth--;
    if(ch===',' && depth===0){ parts.push(token.trim()); token=''; }
    else token+=ch;
  }
...
function dispatch(event){
  const target=event.target?.closest?.('[data-kfe-action]');
  if(!target) return;
  const expected=target.dataset.kfeEvent || 'click';
  ...
  const handler=window.KFE_ACTIONS?.[action];
  if(typeof handler!=='function') return;
  handler(...parseArgs(target.dataset.kfeArgs,event));
}
