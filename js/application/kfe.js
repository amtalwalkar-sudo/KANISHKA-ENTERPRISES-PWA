...
export function createKfeApplication(repository){
 ...
 async function recordFuel(data,operationId=createOperationId()){return withIdempotency(repository,operationId,async()=>{const odometer=Number(data.odometer);if(!Number.isInteger(odometer)||odometer<0)throw new RangeError('Fuel odometer must be a whole number');const latest=await work.latestOdometer();if(latest&&odometer<Number(latest.odometer))throw new RangeError('Fuel odometer cannot be below the authoritative odometer');const price=Number(data.price_per_kg),amount=Number(data.amount_paise);if(!Number.isFinite(price)||price<=0)throw new RangeError('Fuel price must be positive');if(!Number.isInteger(amount)||amount<=0)throw new RangeError('Fuel amount must be positive');return create('fuel_records',{...data,odometer,quantity_kg:amount/100/price,fuel_type:'CNG'},{});});}
 async function updateFuel(id,changes,operationId=createOperationId()){return withIdempotency(repository,operationId,async()=>{const existing=await repository.entity('fuel_records').get(id);if(!existing)throw new Error('Fuel record not found');const amount=Number(changes.amount_paise),price=Number(changes.price_per_kg);if(!Number.isInteger(amount)||amount<=0||!Number.isFinite(price)||price<=0)throw new RangeError('Fuel amount and price must be positive');const latest=await work.latestOdometer();const odometer=Number(changes.odometer);if(!Number.isInteger(odometer)||odometer<0||(latest&&odometer<Number(latest.odometer)))throw new RangeError('Fuel odometer cannot be below the authoritative odometer');const next={...changes,odometer,quantity_kg:amount/100/price,fuel_type:'CNG'};return repository.entity('fuel_records').update(existing,next);});}
 ...
 return Object.freeze({graph,work,telemetry,startWork,getWork,listWork,completeWork,listFuel,recordFuel,updateFuel,undoFuel,listTrips,recordTrip,updateTrip,undoTrip,recordRevenue,getStatus,getTimeline,resetAllData,calculateWorkSession,expectedTomorrowKm,rollingFuelCostPerKm,projectedFuelCostForKm,businessExpenses,fixedExpensePerBusinessKm,provisionMaintenance,reconcileInvoice,maintenanceAlerts,businessRevenue,amortize,applyPrepayment,profitability,tomorrowTarget,evaluateAlerts,getWorkScreenState:work.state,startDay:work.startDay,startShift:work.startShift,startBusinessTrip:work.startBusinessTrip,startPersonalTrip:work.startPersonalTrip,endBusinessTrip:work.endBusinessTrip,endPersonalTrip:work.endPersonalTrip,endShift:work.endShift,endDay:work.endDay,undoWorkAction:work.undo,workSummary:work.summary,latestWorkOdometer:work.latestOdometer});
}
