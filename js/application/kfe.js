...
export function createKfeApplication(repository){
 ...
 async function updateFuel(id,changes,operationId=createOperationId()){return withIdempotency(repository,operationId,async()=>{const existing=await repository.entity('fuel_records').get(id);if(!existing)throw new Error('Fuel record not found');const updated=repository.updateRecord(existing,{...changes,quantity_kg:Number(changes.amount_paise)/100/Number(changes.price_per_kg),fuel_type:'CNG'});repository.assertRecord(updated);await repository.entity('fuel_records').update(existing,updated);return updated;});}
