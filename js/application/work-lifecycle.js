...
export function createWorkApplication({repository,telemetry}){
  ...
  async function endShift({id,endOdometer,revenuePaise,tollPaise=0,parkingPaise=0,tollIncludedInFare=false,parkingIncludedInFare=false,actionMode='SWIPE',direction='RIGHT'}={},operationId=createOperationId()){
  ...
  async function undo(action={},operationId=createOperationId()){
    return withIdempotency(repository,operationId,async()=>{const type=action.type,id=action.id,occurredAt=utcNow();if(!type||!id)throw new Error('Undo action is incomplete');let updated=null;
      ...
      }else if(type==='END_SHIFT'){const shift=await shifts.get(id);if(!shift||shift.status!=='COMPLETED')throw new Error('Shift is not closed');const revenueRows=active(await revenues.list()).filter(x=>x.work_session_id===id&&x.entry_source==='SHIFT_CLOSURE');const reopened=repository.updateRecord(shift,{status:'OPEN',ended_at:null,end_odometer:null,toll_paise:0,parking_paise:0,toll_included_in_fare:false,parking_included_in_fare:false});updated=reopened;await repository.atomic(['work_sessions','revenue_records'],stores=>{stores.work_sessions.put(reopened);for(const row of revenueRows)stores.revenue_records.delete(row.id);return true;});
      }else if(type==='START_TRIP'){const trip=await trips.get(id);if(!trip||trip.status!=='OPEN')throw new Error('Trip is not active');updated=await trips.update(trip,{status:'CANCELLED',ended_at:occurredAt});
      }else if(type==='END_TRIP'){const trip=await trips.get(id);if(!trip||trip.status!=='COMPLETED')throw new Error('Trip is not closed');updated=await trips.update(trip,{status:'OPEN',ended_at:null,duration_seconds:null});
      }else if(type==='START_PERSONAL_TRIP'){const trip=await trips.get(id);if(!trip||trip.scope!=='PERSONAL'||trip.status!=='OPEN')throw new Error('Personal trip is not active');const rows=active(await allocations.list()).filter(x=>x.reference_id===id);updated=await trips.update(trip,{status:'CANCELLED',ended_at:occurredAt});for(const row of rows)await allocations.softDelete(row);
      }else if(type==='END_PERSONAL_TRIP'){const trip=await trips.get(id);if(!trip||trip.scope!=='PERSONAL'||trip.status!=='COMPLETED')throw new Error('Personal trip is not closed');updated=await trips.update(trip,{status:'OPEN',ended_at:null,duration_seconds:null,end_odometer:null});
      }else throw new Error(`Unsupported undo action: ${type}`);
      await recordTelemetry('UNDO',type,id,{actionMode:action.actionMode||'SWIPE',direction:action.direction||'LEFT',occurredAt});await syncTracking();return updated;});
  }

  return {state,startDay,startShift,startBusinessTrip,startPersonalTrip,endBusinessTrip,endPersonalTrip,endShift,endDay,undo};
}
