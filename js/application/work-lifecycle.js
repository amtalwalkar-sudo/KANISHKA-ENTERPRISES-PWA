...
export function createWorkApplication({repository,telemetry}){
  ...
  async function endShift({id,endOdometer,tollPaise=0,parkingPaise=0,tollIncludedInFare=false,parkingIncludedInFare=false,actionMode='SWIPE',direction='RIGHT'}={},operationId=createOperationId()){
    return withIdempotency(repository,operationId,async()=>{const existing=await shifts.get(id);if(!existing||existing.scope!=='BUSINESS'||existing.status!=='OPEN')throw new Error('Shift is not active');const context=await currentContext(existing.business_date||localDate());if(!canEndShift(context)||context.shift?.id!==id)throw new Error('Shift can only end while waiting for a ride');const end=whole(endOdometer,'End odometer');validateWorkOdometer(end,Number(existing.start_odometer));const occurredAt=utcNow();const tripRows=active(await trips.list()).filter(t=>t.scope==='BUSINESS'&&t.shift_id===id&&t.status==='COMPLETED');const updated=await shifts.update(existing,{status:'COMPLETED',ended_at:occurredAt,end_odometer:end,toll_paise:Math.max(0,whole(tollPaise,'Toll')),parking_paise:Math.max(0,whole(parkingPaise,'Parking')),toll_included_in_fare:Boolean(tollIncludedInFare),parking_included_in_fare:Boolean(parkingIncludedInFare),trip_count:tripRows.length});await recordTelemetry('END_SHIFT','WORK_SESSION',updated.id,{actionMode,direction,occurredAt});await syncTracking();return updated;});
  }
  ...
  async function undo(action={},operationId=createOperationId()){
    return withIdempotency(repository,operationId,async()=>{const type=action.type,id=action.id,occurredAt=utcNow();if(!type||!id)throw new Error('Undo action is incomplete');let updated=null;
      ...
      }else if(type==='END_SHIFT'){const shift=await shifts.get(id);if(!shift||shift.status!=='COMPLETED')throw new Error('Shift is not closed');updated=await shifts.update(shift,{status:'OPEN',ended_at:null,end_odometer:null,toll_paise:0,parking_paise:0,toll_included_in_fare:false,parking_included_in_fare:false});
