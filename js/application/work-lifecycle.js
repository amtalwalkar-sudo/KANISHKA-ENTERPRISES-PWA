...
export function createWorkApplication({repository,telemetry}){
  ...
  async function startShift({startOdometer=null,actionMode='SWIPE',direction='RIGHT'}={},operationId=createOperationId()){
    return withIdempotency(repository,operationId,async()=>{const occurredAt=utcNow(),businessDate=localDate(),context=await currentContext(businessDate);if(!canStartShift(context))throw new Error('Shift can only start from the day-ready state');if(context.latest==null)throw new Error('Start day odometer is required before starting a shift');const current=startOdometer==null?Number(context.latest.odometer):whole(startOdometer,'Shift start odometer');validateWorkOdometer(current,Number(context.latest.odometer));const shift=createRecord({business_date:businessDate,scope:'BUSINESS',status:'OPEN',started_at:occurredAt,ended_at:null,start_odometer:current,end_odometer:null,break_minutes:0,trip_count:0,toll_paise:0,parking_paise:0,toll_included_in_fare:false,parking_included_in_fare:false});repository.assertRecord(shift);await repository.atomic(['work_sessions'],stores=>{stores.work_sessions.put(shift);return shift;});await recordTelemetry('START_SHIFT','WORK_SESSION',shift.id,{actionMode,direction,occurredAt});await syncTracking();return shift;});
  }
  async function recordBreakMinutes({minutes,actionMode='BUTTON',direction=null}={},operationId=createOperationId()){
    return withIdempotency(repository,operationId,async()=>{const value=whole(minutes,'Break minutes');if(value<0)throw new RangeError('Break minutes must not be negative');const context=await currentContext(localDate());if(!context.shift||context.shift.status!=='OPEN')throw new Error('Break can only be recorded while a business shift is active');if(context.trip)throw new Error('Break cannot be recorded while a trip is active');const occurredAt=utcNow();const updated=await shifts.update(context.shift,{break_minutes:value});await recordTelemetry('RECORD_BREAK_MINUTES','WORK_SESSION',updated.id,{actionMode,direction,occurredAt});await syncTracking();return updated;});
  }
  ...
  async function summary(){
    ...
    const day=active(dayRows).find(x=>x.business_date===businessDate)||null;
    const shiftsToday=active(shiftRows).filter(x=>x.business_date===businessDate&&x.scope==='BUSINESS');
    const tripsToday=active(tripRows).filter(x=>x.business_date===businessDate&&x.scope==='BUSINESS'&&x.status==='COMPLETED');
    const revenuePaise=active(revenueRows).filter(x=>x.business_date===businessDate&&x.scope==='BUSINESS').reduce((sum,x)=>sum+Number(x.amount_paise||0),0);
    const shiftSeconds=shiftsToday.reduce((sum,x)=>x.started_at&&x.ended_at?sum+Math.max(0,Math.floor((Date.parse(x.ended_at)-Date.parse(x.started_at))/1000)):sum,0);
    ...
    const businessAllocatedKm=active(allocationRows).filter(x=>x.context==='DAY_START'&&x.reference_id===day?.id).reduce((sum,x)=>sum+Number(x.business_km||0),0);
    const deadKms=kmsRun==null?null:Math.max(0,kmsRun-businessAllocatedKm);
    return Object.freeze({businessDate,day,shiftCount:shiftsToday.filter(x=>x.status==='COMPLETED').length,tripCount:tripsToday.length,revenuePaise,shiftSeconds,kmsRun,deadKms,targetPaise:null});
  }
  return Object.freeze({state,startDay,startShift,recordBreakMinutes,startBusinessTrip,startPersonalTrip,endBusinessTrip,endPersonalTrip,endShift,endDay,undo,summary,latestOdometer});
}
