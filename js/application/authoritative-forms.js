...
export const AUTHORITATIVE_FORM_CONTRACT=Object.freeze({
 modeField:'mode',modes:FORM_MODES,requiredCapabilities:Object.freeze(['validate','save','update']),
 rules:Object.freeze({
...
export function createFormContext({type,mode='CREATE',record=null}={}){if(!AUTHORITATIVE_FORM_TYPES.includes(type))throw new RangeError(`Unsupported KFE form type: ${type}`);assertFormMode(mode);if(mode==='EDIT'&&(!record||!record.id))throw new TypeError('EDIT mode requires an existing authoritative record');return Object.freeze({type,mode,record:record||null});}
